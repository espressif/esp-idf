...
typedef union {
    struct {
         ...
         *  When SPI1_MEM_WAITI_ADDR_EN is set, the  cycle length of sent out address is
         *  (SPI1_MEM_WAITI_ADDR_CYCLELEN[1:0] + 1) SPI  bus clock cycles. It is not active
         *  when SPI1_MEM_WAITI_ADDR_EN is cleared.
         */
    ...
    uint32_t val;
} spi1_mem_flash_waiti_ctrl_reg_t;
...
typedef union {
    struct {
         ...
         *  1: SPI1 waits (SPI1_MEM_CS_HOLD_DELAY_RES[9:0] * 4 or *128) SPI_CLK cycles after
         ...
         *  When SPI1 checks SUS/SUS1/SUS2 bits fail for SPI1_MEM_SUS_TIMEOUT_CNT[6:0] times,
         *  it will be treated as check pass.
         */
        uint32_t sus_timeout_cnt:7;
    };
    uint32_t val;
} spi1_mem_flash_sus_ctrl_reg_t;
...
typedef union {
    struct {
         ...
         *  1: SPI1 sends out SPI1_MEM_WAIT_PESR_COMMAND[15:0] to check SUS/SUS1/SUS2 bit. 0:
         *  SPI1 sends out SPI1_MEM_WAIT_PESR_COMMAND[7:0] to check SUS/SUS1/SUS2 bit.
         */
        uint32_t wait_pesr_cmd_2b:1;
        /** flash_hpm_dly_128 : R/W; bitpos: [2]; default: 0;
         *  1: SPI1 waits (SPI1_MEM_CS_HOLD_DELAY_RES[9:0] * 128) SPI_CLK cycles after HPM
         *  command is sent. 0: SPI1 waits (SPI1_MEM_CS_HOLD_DELAY_RES[9:0] * 4) SPI_CLK cycles
         *  after HPM command is sent.
         */
        uint32_t flash_hpm_dly_128:1;
        /** flash_res_dly_128 : R/W; bitpos: [3]; default: 0;
         *  1: SPI1 waits (SPI1_MEM_CS_HOLD_DELAY_RES[9:0] * 128) SPI_CLK cycles after RES
         *  command is sent. 0: SPI1 waits (SPI1_MEM_CS_HOLD_DELAY_RES[9:0] * 4) SPI_CLK cycles
         *  after RES command is sent.
         */
        uint32_t flash_res_dly_128:1;
        /** flash_dp_dly_128 : R/W; bitpos: [4]; default: 0;
         *  1: SPI1 waits (SPI1_MEM_CS_HOLD_DELAY_RES[9:0] * 128) SPI_CLK cycles after DP
         *  command is sent. 0: SPI1 waits (SPI1_MEM_CS_HOLD_DELAY_RES[9:0] * 4) SPI_CLK cycles
         *  after DP command is sent.
         */
        uint32_t flash_dp_dly_128:1;
        /** flash_per_dly_128 : R/W; bitpos: [5]; default: 0;
         *  Valid when SPI1_MEM_FLASH_PER_WAIT_EN is 1. 1: SPI1 waits
         *  (SPI1_MEM_CS_HOLD_DELAY_RES[9:0] * 128) SPI_CLK cycles after PER command is sent.
         *  0: SPI1 waits (SPI1_MEM_CS_HOLD_DELAY_RES[9:0] * 4) SPI_CLK cycles after PER
         *  command is sent.
         */
        uint32_t flash_per_dly_128:1;
        /** flash_pes_dly_128 : R/W; bitpos: [6]; default: 0;
         *  Valid when SPI1_MEM_FLASH_PES_WAIT_EN is 1. 1: SPI1 waits
         *  (SPI1_MEM_CS_HOLD_DELAY_RES[9:0] * 128) SPI_CLK cycles after PES command is sent.
         *  0: SPI1 waits (SPI1_MEM_CS_HOLD_DELAY_RES[9:0] * 4) SPI_CLK cycles after PES
         *  command is sent.
         */
...
typedef union {
    struct {
         ...
         */
        uint32_t f_hyperbus_dummy_2x:1;
        /** f_clk_diff_inv : HRO; bitpos: [28]; default: 0;
         *  Set this bit to invert SPI_DIFF when accesses to flash. .
         */
        uint32_t f_clk_diff_inv:1;
        /** f_octa_ram_addr : HRO; bitpos: [29]; default: 0;
         ...
         */
        uint32_t f_octa_ram_addr:1;
        /** f_hyperbus_ca : HRO; bitpos: [30]; default: 0;
...
typedef union {
    struct {
         ...
         *  The raw bit for SPI1_MEM_PES_END_INT interrupt.1: Triggered when Auto Suspend
         ...
         *  The raw bit for SPI1_MEM_WPE_END_INT interrupt. 1: Triggered when WRSR/PP/SE/BE/CE
         *  is sent and flash is already idle. 0: Others.
         */
        uint32_t wpe_end_int_raw:1;
        /** slv_st_end_int_raw : R/WTC/SS; bitpos: [3]; default: 0;
         *  The raw bit for SPI1_MEM_SLV_ST_END_INT interrupt. 1: Triggered when spi1_slv_st is
         ...
         *  The raw bit for SPI1_MEM_MST_ST_END_INT interrupt. 1: Triggered when spi1_mst_st is
         ...
         *  The raw bit for SPI1_MEM_BROWN_OUT_INT interrupt. 1: Triggered condition is that
...
typedef union {
    struct {
         ...
         *  The status bit for SPI1_MEM_PES_END_INT interrupt.
         */
        uint32_t pes_end_int_st:1;
        /** wpe_end_int_st : RO; bitpos: [2]; default: 0;
         *  The status bit for SPI1_MEM_WPE_END_INT interrupt.
         */
        uint32_t wpe_end_int_st:1;
        /** slv_st_end_int_st : RO; bitpos: [3]; default: 0;
         *  The status bit for SPI1_MEM_SLV_ST_END_INT interrupt.
         */
        uint32_t slv_st_end_int_st:1;
        /** mst_st_end_int_st : RO; bitpos: [4]; default: 0;
         *  The status bit for SPI1_MEM_MST_ST_END_INT interrupt.
         */
...
typedef struct spi1_mem_dev_t {
    ...
    volatile spi1_mem_ctrl1_reg_t ctrl1;
    volatile spi1_mem_ctrl2_reg_t ctrl2;
    volatile spi1_mem_clock_reg_t clock;
    volatile spi1_mem_user_reg_t user;
    volatile spi1_mem_user1_reg_t user1;
    volatile spi1_mem_user2_reg_t user2;
    volatile spi1_mem_mosi_dlen_reg_t mosi_dlen;
    volatile spi1_mem_miso_dlen_reg_t miso_dlen;
    volatile spi1_mem_rd_status_reg_t rd_status;
    ...
    volatile spi1_mem_sus_status_reg_t sus_status;
    uint32_t reserved_0a8[6];
    volatile spi1_mem_int_ena_reg_t int_ena;
    volatile spi1_mem_int_clr_reg_t int_clr;
    volatile spi1_mem_int_raw_reg_t int_raw;
    volatile spi1_mem_int_st_reg_t int_st;
    uint32_t reserved_0d0;
    volatile spi1_mem_ddr_reg_t ddr;
    uint32_t reserved_0d8[42];
    volatile spi1_mem_timing_cali_reg_t timing_cali;
    uint32_t reserved_184[31];
    volatile spi1_mem_clock_gate_reg_t clock_gate;

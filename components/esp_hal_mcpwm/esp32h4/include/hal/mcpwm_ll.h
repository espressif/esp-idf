...
{
    if (group_id == 0) {
        PCR.pwm0_conf.pwm0_clk_en = enable;
    } else {
        PCR.pwm1_conf.pwm1_clk_en = enable;
    }
}
...
{
    if (group_id == 0) {
        PCR.pwm0_conf.pwm0_rst_en = 1;
        PCR.pwm0_conf.pwm0_rst_en = 0;
    } else {
        PCR.pwm1_conf.pwm1_rst_en = 1;
        PCR.pwm1_conf.pwm1_rst_en = 0;
    }
}
...
{
    if (group_id == 0) {
        PCR.pwm0_clk_conf.pwm0_clkm_en = en;
    } else {
        PCR.pwm1_clk_conf.pwm1_clkm_en = en;
    }
}
...
static inline void mcpwm_ll_group_set_clock_source(int group_id, soc_module_clk_t clk_src)
{
    ...
    if (group_id == 0) {
        PCR.pwm0_clk_conf.pwm0_clkm_sel = clk_id;
    } else {
        PCR.pwm1_clk_conf.pwm1_clkm_sel = clk_id;
    }
}
...
static inline void mcpwm_ll_group_set_clock_prescale(int group_id, int prescale)
{
    ...
    if (group_id == 0) {
        HAL_FORCE_MODIFY_U32_REG_FIELD(PCR.pwm0_clk_conf, pwm0_div_num, prescale - 1);
    } else {
        HAL_FORCE_MODIFY_U32_REG_FIELD(PCR.pwm1_clk_conf, pwm1_div_num, prescale - 1);
    }
}

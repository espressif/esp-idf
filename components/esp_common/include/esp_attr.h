...
// The string is placed into a uniquely-named variable in a uniquely-named
// subsection of .dram1. Both names are derived from a single __COUNTER__ value.
// This avoids "section type conflict" errors under LTO, where the local static
// variables generated by DRAM_STR in different translation units would otherwise
// share the same name and the same DRAM_ATTR section name after the units are merged.
#define DRAM_STR(str) _DRAM_STR_IMPL(str, __COUNTER__)
#define _DRAM_STR_COMBINE(x, y) x ## y
#define _DRAM_STR_IMPL(str, counter) \
    _DRAM_STR_IMPL2(str, _DRAM_STR_COMBINE(__c, counter), _SECTION_ATTR_IMPL(".dram1.str", counter))
#define _DRAM_STR_IMPL2(str, varname, attr) \
    (__extension__({static const attr char varname[] = (str); (const char *)&varname;}))

...
TEST_CASE("mbedtls GCM stream test", "[aes-gcm]")
{
    ...
    for (int bytes_to_process = 16; bytes_to_process < SZ; bytes_to_process = bytes_to_process + 16) {
        ...
        for (int idx = 0; idx < SZ; idx = idx + bytes_to_process) {
            ...
            status = psa_aead_update(&operation, plaintext + idx, length, ciphertext + total_output, SZ - total_output, &output_len);
            TEST_ASSERT_EQUAL(PSA_SUCCESS, status);
            total_output += output_len;
        }

        size_t tag_len;
        status = psa_aead_finish(&operation, ciphertext + total_output, SZ - total_output, &output_len, tag, sizeof(tag), &tag_len);
        TEST_ASSERT_EQUAL(PSA_SUCCESS, status);
        total_output += output_len;
        ...
        for (int idx = 0; idx < SZ; idx = idx + bytes_to_process) {
            // Limit length of last call to avoid exceeding buffer size
            size_t length = (idx + bytes_to_process > SZ) ? (SZ - idx) : bytes_to_process;
            status = psa_aead_update(&operation, ciphertext + idx, length, decryptedtext + total_output, SZ - total_output, &output_len);
            TEST_ASSERT_EQUAL(PSA_SUCCESS, status);
            total_output += output_len;
        }

        status = psa_aead_verify(&operation, decryptedtext + total_output, SZ - total_output, &output_len, tag, sizeof(tag));
        TEST_ASSERT_EQUAL(PSA_SUCCESS, status);
        total_output += output_len;
...
static void aes_gcm_test(aes_gcm_test_cfg_t *cfg, aes_gcm_test_expected_res_t *res)
{
    psa_key_id_t key_id;
    psa_key_attributes_t attributes = PSA_KEY_ATTRIBUTES_INIT;
    psa_status_t status;
    ...
    // Determine the GCM algorithm with appropriate tag length
    psa_algorithm_t gcm_alg;
    if (cfg->tag_len == 16) {
        gcm_alg = PSA_ALG_GCM;
    } else {
        gcm_alg = PSA_ALG_AEAD_WITH_SHORTENED_TAG(PSA_ALG_GCM, cfg->tag_len);
    }
    ...
    psa_set_key_type(&attributes, PSA_KEY_TYPE_AES);
    psa_set_key_bits(&attributes, cfg->key_bits);

    // Import key
    status = psa_import_key(&attributes, cfg->key, cfg->key_bits / 8, &key_id);
    TEST_ASSERT_EQUAL(PSA_SUCCESS, status);

    /* Encrypt and tag */
    psa_aead_operation_t operation = PSA_AEAD_OPERATION_INIT;

    status = psa_aead_encrypt_setup(&operation, key_id, gcm_alg);
    TEST_ASSERT_EQUAL(PSA_SUCCESS, status);

    status = psa_aead_set_nonce(&operation, iv_buf, cfg->iv_length);
    TEST_ASSERT_EQUAL(PSA_SUCCESS, status);

    status = psa_aead_update_ad(&operation, cfg->add_buf, cfg->add_length);
    TEST_ASSERT_EQUAL(PSA_SUCCESS, status);

    size_t output_len;
    status = psa_aead_update(&operation, cfg->plaintext, cfg->plaintext_length, ciphertext, cfg->plaintext_length, &output_len);
    TEST_ASSERT_EQUAL(PSA_SUCCESS, status);

    size_t tag_len;
    status = psa_aead_finish(&operation, ciphertext + output_len, cfg->plaintext_length - output_len, &output_len, tag_buf_encrypt, sizeof(tag_buf_encrypt), &tag_len);
    TEST_ASSERT_EQUAL(PSA_SUCCESS, status);
    TEST_ASSERT_EQUAL(cfg->tag_len, tag_len);
    ...
    status = psa_aead_decrypt_setup(&operation, key_id, gcm_alg);
    TEST_ASSERT_EQUAL(PSA_SUCCESS, status);
    ...
    status = psa_aead_update(&operation, ciphertext, cfg->plaintext_length, output, cfg->plaintext_length, &output_len);
    TEST_ASSERT_EQUAL(PSA_SUCCESS, status);

    status = psa_aead_verify(&operation, output + output_len, cfg->plaintext_length - output_len, &output_len, res->expected_tag, cfg->tag_len);
    TEST_ASSERT_EQUAL(PSA_SUCCESS, status);
...
TEST_CASE("mbedtls AES GCM performance, multi-part", "[aes-gcm]")
{
    ...
    status = psa_aead_set_nonce(&operation, iv, sizeof(iv));
    TEST_ASSERT_EQUAL(PSA_SUCCESS, status);

    status = psa_aead_update_ad(&operation, aad, sizeof(aad));
    TEST_ASSERT_EQUAL(PSA_SUCCESS, status);

    size_t output_len;
    status = psa_aead_update(&operation, buf, CALL_SZ, buf, CALL_SZ, &output_len);
    TEST_ASSERT_EQUAL(PSA_SUCCESS, status);

    size_t tag_len;
    status = psa_aead_finish(&operation, buf + output_len, CALL_SZ - output_len, &output_len, tag_buf, 16, &tag_len);
    TEST_ASSERT_EQUAL(PSA_SUCCESS, status);
...
TEST_CASE("mbedtls AES GCM performance, one-shot", "[aes-gcm]")
{
    const unsigned CALL_SZ = 16 * 3200;
    psa_key_id_t key_id;
    psa_key_attributes_t attributes = PSA_KEY_ATTRIBUTES_INIT;
    ...
    // allocate internal memory with extra space for tag
    uint8_t *buf = heap_caps_malloc(CALL_SZ + 16, MALLOC_CAP_DMA | MALLOC_CAP_8BIT | MALLOC_CAP_INTERNAL);
    TEST_ASSERT_NOT_NULL(buf);
    ...
    ccomp_timer_start();
    size_t output_len;
    status = psa_aead_encrypt(key_id, PSA_ALG_GCM, iv, sizeof(iv), aad, sizeof(aad),
                             buf, CALL_SZ, buf, CALL_SZ + 16, &output_len);
    TEST_ASSERT_EQUAL(PSA_SUCCESS, status);
    ...
    */
    ...
    TEST_ASSERT_EQUAL_HEX8_ARRAY(expected_last_block, buf + CALL_SZ - 16, 16);
    TEST_ASSERT_EQUAL_HEX8_ARRAY(expected_tag, buf + CALL_SZ, 16);
...
/* Note: This test validates different GCM authentication tag lengths.
 * The PSA standard allows tag lengths of 4, 8, 12-16 bytes for GCM.
 */

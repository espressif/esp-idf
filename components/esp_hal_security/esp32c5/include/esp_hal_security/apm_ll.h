...
/* Helper macros for getting APM ctrl region pms attributes field mask */
...
/* Helper macros for getting TEE ctrl periph pms attributes field mask */
#define APM_PERI_PMS_MASK(mode)         ((1U << (mode)) | (1U << ((mode) ^ 4U)))
#define APM_PERI_PMS_R_FIELD(mode, pms) ((((pms) & APM_PERM_R) >> 2U) << (mode))
#define APM_PERI_PMS_W_FIELD(mode, pms) ((((pms) & APM_PERM_W) >> 1U) << ((mode) ^ 4U))
...
/**
 * @brief Set access permissions for a specific peripheral in HP-TEE
 *
 * @param periph Peripheral ID
 * @param mode Security mode
 * @param pms Access permissions
 */
static inline void apm_ll_hp_tee_set_peri_access(apm_tee_hp_periph_t periph, apm_security_mode_t mode, apm_perm_t pms)
{
    uint32_t reg = TEE_UART0_CTRL_REG + APM_TEE_PERI_CTRL_OFFSET * periph;
    uint32_t val = REG_READ(reg);
    val &= ~APM_PERI_PMS_MASK(mode);
    val |= APM_PERI_PMS_R_FIELD(mode, pms);
    val |= APM_PERI_PMS_W_FIELD(mode, pms);
    REG_WRITE(reg, val);
}

/**
 * @brief Enable or disable error response from HP-TEE when access is blocked.
 *
 * @param enable true to enable error response, false to disable.
 */
static inline void apm_ll_hp_tee_enable_bus_err_resp(bool enable)
{
    if (enable) {
        REG_SET_BIT(TEE_BUS_ERR_CONF_REG, TEE_BUS_ERR_RESP_EN);
    } else {
        REG_CLR_BIT(TEE_BUS_ERR_CONF_REG, TEE_BUS_ERR_RESP_EN);
    }
}
...
/**
 * @brief Lock master security mode for LP-TEE
 *
 * @param id Master ID (unused)
 */
static inline void apm_ll_lp_tee_lock_master_sec_mode(apm_master_id_t id)
{
    (void)id;
    REG_SET_BIT(LP_TEE_M0_MODE_CTRL_REG, LP_TEE_M0_LOCK);
}

/**
 * @brief Set access permissions for a specific peripheral in LP-TEE
 *
 * @param periph Peripheral ID
 * @param mode Security mode
 * @param pms Access permissions
 */
static inline void apm_ll_lp_tee_set_peri_access(apm_tee_lp_periph_t periph, apm_security_mode_t mode, apm_perm_t pms)
{
    uint32_t reg = LP_TEE_EFUSE_CTRL_REG + APM_TEE_PERI_CTRL_OFFSET * periph;
    uint32_t val = REG_READ(reg);
    val &= ~APM_PERI_PMS_MASK(mode);
    val |= APM_PERI_PMS_R_FIELD(mode, pms);
    val |= APM_PERI_PMS_W_FIELD(mode, pms);
...
/**
 * @brief Enable or disable error response from LP-TEE when access is blocked.
 *
 * @param enable true to enable error response, false to disable.
 */
static inline void apm_ll_lp_tee_enable_bus_err_resp(bool enable)
{
    if (enable) {
        REG_SET_BIT(LP_TEE_BUS_ERR_CONF_REG, LP_TEE_BUS_ERR_RESP_EN);
    } else {
        REG_CLR_BIT(LP_TEE_BUS_ERR_CONF_REG, LP_TEE_BUS_ERR_RESP_EN);
    }
}
...
{
    uint32_t reg = LP_APM0_REGION0_ATTR_REG + APM_REGION_ATTR_OFFSET * regn_num;
    uint32_t val = REG_READ(reg);
    val &= ~APM_REGION_PMS_MASK(mode);
    val |= APM_REGION_PMS_FIELD(mode, regn_pms);
    REG_WRITE(reg, val);
...
/**
 * @brief Lock security mode region attributes in LP-APM0
 *
 * @param regn_num Region number
 */
static inline void apm_ll_lp_apm0_lock_sec_mode_region_attr(uint32_t regn_num)
{
    REG_SET_BIT(LP_APM0_REGION0_ATTR_REG + APM_REGION_ATTR_OFFSET * regn_num, APM_REGION_LOCK_BIT);
}

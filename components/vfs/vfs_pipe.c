...
//#define VFS_PIPE_DEBUG(tag, fmt, ...) fprintf(stderr, "[%s] " fmt "\n", pcTaskGetName(NULL), ##__VA_ARGS__)
...
static ssize_t pipe_write(int fd, const void *data, size_t size)
{
    ...
    VFS_PIPE_DEBUG(TAG, "pipe_write(local_fd=%d, idx=%zu size=%zu)", fd, idx, size);
...
static ssize_t pipe_read(int fd, void *data, size_t size)
{
    ...
    VFS_PIPE_DEBUG(TAG, "pipe_read(local_fd=%d, idx=%zu size=%zu)", fd, idx, size);
...
static int pipe_fcntl(int fd, int cmd, int flags)
{
    ...
    VFS_PIPE_DEBUG(TAG, "pipe_fcntl(local_fd=%d, idx=%zu flags=%d)", fd, idx, flags);
    pipe_context_t *pipe = &s_pipes[idx];
    _lock_acquire(&pipe->lock);
...
static int pipe_close(int fd)
{
    ...
    VFS_PIPE_DEBUG(TAG, "pipe_close(local_fd=%d, idx=%zu)", fd, idx);
    ...
    if (IS_WRITE(fd)) {
        if (pipe->write_fd == FD_INVALID) {
            VFS_PIPE_DEBUG(TAG, "pipe_close: write fd=%d is already closed", pipe->write_fd);
            res = -1;
        }
        pipe->write_fd = FD_INVALID;

    } else {
        if (pipe->read_fd == FD_INVALID) {
            VFS_PIPE_DEBUG(TAG, "pipe_close: read fd=%d is already closed", pipe->read_fd);
            res = -1;
        }
...
int pipe(int fildes[2])
{
    ...
    for (size_t i = 0; i < s_pipes_size; i++) {
        ...
        if (s_pipes[i].read_fd == FD_INVALID && s_pipes[i].write_fd == FD_INVALID) {
            ...
            VFS_PIPE_DEBUG(TAG, "pipe() -> read fd=%d local_fd=%d, write fd=%d local_fd=%d", global_read_fd, IDX_TO_FD_READ(i), global_write_fd, IDX_TO_FD_WRITE(i));

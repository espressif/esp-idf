...
TEST_CASE("nvs_flash_init_partition_ptr inits one partition", "[nvs_custom_part]")
{
    ...
    nvs_flash_deinit_partition("test");

    CHECK(nvs_open_from_partition("test", "test_ns", NVS_READWRITE, &out_handle) == ESP_ERR_NVS_PART_NOT_FOUND);

    CHECK(nvs_flash_init_partition_ptr(&partition) == ESP_OK);
    CHECK(nvs_open_from_partition("test", "test_ns", NVS_READWRITE, &out_handle) == ESP_OK);
    CHECK(nvs_flash_deinit_partition("test") == ESP_OK);
}

TEST_CASE("nvs_flash_init_partition_ptr deinits one partition", "[nvs_custom_part]")
{
    const uint32_t NVS_FLASH_SECTOR = 6;
    const uint32_t NVS_FLASH_SECTOR_COUNT_MIN = 3;
    SpiFlashEmulator emu(10);

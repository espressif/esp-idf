...
static int count_blob_data_chunk(nvs::Partition *partition, uint32_t sectorCount, uint8_t nsIndex, const char *key, uint8_t chunkIndex)
{
    int count = 0;
    for (uint32_t sector = 0; sector < sectorCount; ++sector) {
        nvs::Page page;
        if (page.load(partition, sector) != ESP_OK) {
            continue;
        }
...
TEST_CASE("Regression: failed multi-page blob update must not leak new-version chunks", "[nvs_storage]")
{
    ...
    PartitionEmulationFixture f(0, sectorCount, TEST_DEFAULT_PARTITION_NAME);
    ...
    REQUIRE(storage.writeItem(ns, nvs::ItemType::BLOB, "blob", original.data(), blob_size) == ESP_OK);
    REQUIRE(count_blob_data_chunk(f.part(), sectorCount, ns, "blob", 0x00) == 1);
    REQUIRE(count_blob_data_chunk(f.part(), sectorCount, ns, "blob", 0x01) == 1);
    ...
    for (int i = 0; ; ++i) {
        ...
        REQUIRE(storage.writeItem(ns, nvs::ItemType::U8, filler_key, &filler_value, sizeof(filler_value)) == ESP_OK);
    }
    ...
    esp_err_t err = storage.writeItem(ns, nvs::ItemType::BLOB, "blob", updated.data(), blob_size);
    ...
    CHECK(count_blob_data_chunk(f.part(), sectorCount, ns, "blob", 0x80) == 0);
    CHECK(count_blob_data_chunk(f.part(), sectorCount, ns, "blob", 0x81) == 0);
...
TEST_CASE("Regression: failed multi-page blob write must not leak a chunk relocated by reclamation", "[nvs_storage]")
{
    ...
    for (int i = 0; i < 100; ++i) {
        ...
        REQUIRE(storage.writeItem(ns, nvs::ItemType::U8, key, &small_value, sizeof(small_value)) == ESP_OK);
    }
    for (int i = 2; i < 100; ++i) {
        char key[16];
        snprintf(key, sizeof(key), "small%03d", i);
        REQUIRE(storage.eraseItem(ns, nvs::ItemType::U8, key) == ESP_OK);
    }
    ...
    esp_err_t err = storage.writeItem(ns, nvs::ItemType::BLOB, "big", blob.data(), blob_size);
    CHECK(err == ESP_ERR_NVS_NOT_ENOUGH_SPACE);

    // Intended failure point: the rollback must remove every chunk of the aborted blob, including the
    // relocated first chunk (0x00).
    CHECK(count_blob_data_chunk(f.part(), sectorCount, ns, "big", 0x00) == 0);
    CHECK(count_blob_data_chunk(f.part(), sectorCount, ns, "big", 0x01) == 0);
    CHECK(count_blob_data_chunk(f.part(), sectorCount, ns, "big", 0x02) == 0);
}
...
TEST_CASE("Regression: failed multi-page blob update must not destroy the previous value", "[nvs_storage]")
{
    ...
    CHECK(count_blob_data_chunk(f.part(), sectorCount, ns, "blob", 0x00) == 1);

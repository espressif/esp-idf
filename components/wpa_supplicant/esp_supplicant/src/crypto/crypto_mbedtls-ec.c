                                      ...
                                      mbedtls_ecp_point *res)
{
    ...
    ecc_point_t p_hw = { 0 };
    ecc_point_t r_hw = { 0 };
    unsigned char scalar_le[P256_LEN] = { 0 };
    ...
    p_hw.len = curve_len;
    MBEDTLS_MPI_CHK(mbedtls_mpi_write_binary_le(&p->MBEDTLS_PRIVATE(X), p_hw.x, P256_LEN));
    MBEDTLS_MPI_CHK(mbedtls_mpi_write_binary_le(&p->MBEDTLS_PRIVATE(Y), p_hw.y, P256_LEN));
    MBEDTLS_MPI_CHK(mbedtls_mpi_write_binary_le(k, scalar_le, P256_LEN));
...
{
    if (mbedtls_pk_check_pair((mbedtls_pk_context *)key1, (mbedtls_pk_context *)key2, crypto_rng_wrapper, NULL) < 0) {
        return 0;
    }
    return 1;
}
...
struct crypto_key * crypto_ec_set_pubkey_point(const struct crypto_ec_group *group,
                                               const u8 *buf, size_t len)
{
    return (struct crypto_key *) crypto_ec_key_set_pub(group, buf, len);
}
...
{
    return crypto_ec_key_get_public_key((struct crypto_ec_key *) key);
}
...
{
    return crypto_ec_key_get_private_key((struct crypto_ec_key *) key);
}
...
static int crypto_ecdh_fast_p256(struct crypto_key *key_own,
...
static int crypto_ecdh_generic(struct crypto_key *key_own,
                               struct crypto_key *key_peer,
                               u8 *secret, size_t *secret_len)
{
    mbedtls_pk_context *own = (mbedtls_pk_context *)key_own;
                          ...
                          const struct crypto_bignum *r, const struct crypto_bignum *s, struct crypto_key *csign, int hash_len)
{
    ...
    ret = mbedtls_ecdsa_sign(&ctx->MBEDTLS_PRIVATE(grp), (mbedtls_mpi *)r, (mbedtls_mpi *)s,
                             &ctx->MBEDTLS_PRIVATE(d), hash, hash_len, crypto_rng_wrapper, NULL);
...
{
    crypto_ec_key_debug_print((struct crypto_ec_key *) key, title);
}
...
{
    return (struct crypto_key *) crypto_ec_key_gen(ike_group);
}
...
struct wpabuf * crypto_ec_key_get_subject_public_key(struct crypto_ec_key *key)
{
    ...
    der_len = crypto_ec_write_pub_key((struct crypto_key *) key, &der);
    if (!der) {
...
struct crypto_ecdh * crypto_ecdh_init(int group)
{
    ...
    if (mbedtls_ecdh_gen_public(ACCESS_ECDH(&ctx, grp), ACCESS_ECDH(&ctx, d), ACCESS_ECDH(&ctx, Q), crypto_rng_wrapper, NULL) != 0) {
                                        ...
                                        const u8 *key, size_t len)
{
    ...
    /* Calculate secret
    z = F(DH(x,Y)) */
    secret_key = mbedtls_ecdh_calc_secret(ctx, &olen, secret, len_prime, crypto_rng_wrapper, NULL);
    if (secret_key != 0) {

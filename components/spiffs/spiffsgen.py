...
class SpiffsFS:
    ...
    def create_file(self, img_path, file_path):  # type: (str, str) -> None
        ...
        if len(img_path) > self.build_config.obj_name_len - 1:
            raise RuntimeError(f"object name '{img_path}' too long")
...
def main():  # type: () -> None
    ...
    parser.add_argument(
        '--page-size', help='Logical page size. Set to value same as CONFIG_SPIFFS_PAGE_SIZE.', type=int, default=256
    )
    ...
    parser.add_argument(
        '--meta-len', help='File metadata length. Set to value same as CONFIG_SPIFFS_META_LENGTH.', type=int, default=4
    )
    ...
    parser.add_argument(
        ...
        dest='use_magic_len',
        help=(
            'Use position in memory to create different magic numbers for each block. '

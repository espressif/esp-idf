...
{
    if (i2c_port == 0) {
        PCR.i2c0_conf.i2c0_clk_en = enable;
    } else if (i2c_port == 1) {
        PCR.i2c1_conf.i2c1_clk_en = enable;
    } else {
        HAL_ASSERT(false);
    }
}
...
{
    if (i2c_port == 0) {
        PCR.i2c0_conf.i2c0_rst_en = 1;
        PCR.i2c0_conf.i2c0_rst_en = 0;
    } else if (i2c_port == 1) {
        PCR.i2c1_conf.i2c1_rst_en = 1;
        PCR.i2c1_conf.i2c1_rst_en = 0;
    } else {
        HAL_ASSERT(false);
    }
}
...
{
    if (I2C_LL_GET_NUM(hw) == 0) {
        HAL_FORCE_MODIFY_U32_REG_FIELD(PCR.i2c0_sclk_conf, i2c0_sclk_div_num, bus_cfg->clkm_div - 1);
    } else if (I2C_LL_GET_NUM(hw) == 1) {
        HAL_FORCE_MODIFY_U32_REG_FIELD(PCR.i2c1_sclk_conf, i2c1_sclk_div_num, bus_cfg->clkm_div - 1);
    } else {
        HAL_ASSERT(false);
    }
...
static inline void i2c_ll_master_set_fractional_divider(i2c_dev_t *hw, uint8_t div_a, uint8_t div_b)
{
    ...
    if (I2C_LL_GET_NUM(hw) == 0) {
        HAL_FORCE_MODIFY_U32_REG_FIELD(PCR.i2c0_sclk_conf, i2c0_sclk_div_a, div_a);
        HAL_FORCE_MODIFY_U32_REG_FIELD(PCR.i2c0_sclk_conf, i2c0_sclk_div_b, div_b);
    } else if (I2C_LL_GET_NUM(hw) == 1) {
        HAL_FORCE_MODIFY_U32_REG_FIELD(PCR.i2c1_sclk_conf, i2c1_sclk_div_a, div_a);
        HAL_FORCE_MODIFY_U32_REG_FIELD(PCR.i2c1_sclk_conf, i2c1_sclk_div_b, div_b);
    } else {
        HAL_ASSERT(false);
    }
}
...
static inline void i2c_ll_set_source_clk(i2c_dev_t *hw, i2c_clock_source_t src_clk)
{
    ...
    if (I2C_LL_GET_NUM(hw) == 0) {
        PCR.i2c0_sclk_conf.i2c0_sclk_sel = (src_clk == I2C_CLK_SRC_RC_FAST) ? 1 : 0;
    } else if (I2C_LL_GET_NUM(hw) == 1) {
        PCR.i2c1_sclk_conf.i2c1_sclk_sel = (src_clk == I2C_CLK_SRC_RC_FAST) ? 1 : 0;
    } else {
        HAL_ASSERT(false);
    }
}
...
static inline void i2c_ll_enable_controller_clock(i2c_dev_t *hw, bool en)
{
    (void)hw;
    if (I2C_LL_GET_NUM(hw) == 0) {
        PCR.i2c0_sclk_conf.i2c0_sclk_en = en;
    } else if (I2C_LL_GET_NUM(hw) == 1) {
        PCR.i2c1_sclk_conf.i2c1_sclk_en = en;
    } else {
        HAL_ASSERT(false);
    }
}

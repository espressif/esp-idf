                              ...
                              int vol_h, int vol_l, int vol_a, int slope, bool is_conn_gnd)
{
    ...
    // TEST_ESP_OK( touch_pad_config(pad_num) );
    for (int i = 0; i < TEST_TOUCH_CHANNEL; i++) {
        TEST_ESP_OK( touch_pad_config(touch_list[i]) );
    }
    ...
    TEST_ESP_OK( touch_pad_set_inactive_connect(is_conn_gnd) );
...
{
    touch_pad_proximity_t proximity;
    uint16_t ch_mask = 0;
    touch_event_t evt = {0};
    int ret = ESP_FAIL;

    TEST_ESP_OK( touch_pad_proximity_get_config(&proximity) );
    printf("Active: ");
    while (1) {
        if (pdTRUE == xQueueReceive(que_touch, &evt, exceed_time_ms / portTICK_PERIOD_MS)) {
            if (evt.intr_mask & TOUCH_PAD_INTR_MASK_ACTIVE) {
                ...
                for (int i = TOUCH_PAD_MAX - 1; i >= 0; i--) {
                    if (BIT(i) & ch_mask) {
                        if (evt.pad_num == i) {
                            if (proximity.meas_num == evt.slp_proxi_cnt) {
...
static esp_err_t test_touch_check_ch_released_with_proximity(uint32_t test_ch_num, uint32_t exceed_time_ms)
{
    ...
    touch_pad_proximity_get_config(&proximity);
    printf("Inactive: ");
    while (1) {
        if (pdTRUE == xQueueReceive(que_touch, &evt, exceed_time_ms / portTICK_PERIOD_MS)) {
            if (evt.intr_mask & TOUCH_PAD_INTR_MASK_INACTIVE) {
...
static void test_touch_intr_cb(void *arg)
{
    ...
    if (evt.intr_mask & TOUCH_PAD_INTR_MASK_SCAN_DONE) {
        ...
        touch_pad_sleep_channel_read_proximity_cnt(&cnt);
...
esp_err_t test_touch_timeout_interrupt(void)
{
    ...
    TEST_ESP_OK( touch_pad_timeout_disable() );
    TEST_ESP_OK( touch_pad_timeout_set_threshold(touch_value * 10) );
    TEST_ESP_OK( touch_pad_timeout_enable() );
...
esp_err_t test_touch_proximity(int meas_num)
{
    ...
    touch_pad_proximity_t proximity = {
        .select_pad[0] = proximity_pad[0],
        .select_pad[1] = proximity_pad[1],
        .select_pad[2] = proximity_pad[2],
        .meas_num = meas_num < 0 ? 16 : meas_num,
    };
    /* Set the threshold. */
    for (int i = 0; i < TEST_TOUCH_CHANNEL; i++) {
        TEST_ESP_OK( touch_pad_filter_read_baseline(touch_list[i], &touch_value) );
        if (touch_list[i] == proximity.select_pad[0] ||
                touch_list[i] == proximity.select_pad[1] ||
                touch_list[i] == proximity.select_pad[2]) {
            /* The threshold of proximity pad is the sum of touch reading `meas_num` times */
            TEST_ESP_OK( touch_pad_set_thresh(touch_list[i],
                                              proximity.meas_num * touch_value * (1 + TOUCH_INTR_THRESHOLD)) );
            ESP_LOGI(TAG, "proximity pad [%d] base %d, thresh %d", touch_list[i], touch_value,
                     (uint32_t)(proximity.meas_num * touch_value * (1 + TOUCH_INTR_THRESHOLD)));
        } else {
            TEST_ESP_OK( touch_pad_set_thresh(touch_list[i], touch_value * TOUCH_INTR_THRESHOLD) );
    ...
    /* Proximity function */
    TEST_ESP_OK( touch_pad_proximity_set_config(&proximity) );
...
esp_err_t test_touch_sleep_reading_stable(touch_pad_t sleep_pad)
{
    ...
    TEST_ESP_OK( touch_pad_sleep_channel_set_config(&slp_config) );
    /* Register touch interrupt ISR, enable intr type. */
    TEST_ESP_OK( touch_pad_intr_enable(TOUCH_PAD_INTR_MASK_ACTIVE | TOUCH_PAD_INTR_MASK_INACTIVE) );
    TEST_ESP_OK( touch_pad_set_fsm_mode(TOUCH_FSM_MODE_TIMER) );
    TEST_ESP_OK( touch_pad_fsm_start() );
    ...
    TEST_ESP_OK( touch_pad_sleep_channel_read_baseline(&touch_value) );
    TEST_ESP_OK( touch_pad_sleep_set_threshold(touch_value * TOUCH_INTR_THRESHOLD) );
    vTaskDelay(50 * SYS_DELAY_TIME_MOM / portTICK_PERIOD_MS);

    while (test_cnt--) {
        /* Touch reading filtered value equal to raw data. */
        for (int i = 0; i < TEST_TOUCH_CHANNEL; i++) {
            TEST_ESP_OK( touch_pad_sleep_channel_read_data(&touch_value) );
            TEST_ESP_OK( touch_pad_sleep_channel_read_baseline(&touch_temp) );
            TEST_ASSERT_UINT32_WITHIN(TOUCH_READ_ERROR, touch_temp, touch_value);
            TEST_ESP_OK( touch_pad_sleep_channel_read_smooth(&touch_temp) );
            TEST_ASSERT_UINT32_WITHIN(TOUCH_READ_ERROR, touch_temp, touch_value);
        }
        for (int i = 0; i < TEST_TOUCH_CHANNEL; i++) {
            if (touch_temp) {
    ...
    TEST_ESP_OK( touch_pad_sleep_channel_read_baseline(&ret_val) );
...
{
    uint32_t touch_value, smooth, ret_val, dbc;
    ...
    if (is_proximity) {
        ...
        for (int i = 0; i < TEST_TOUCH_CHANNEL; i++) {
            if (touch_list[i] == sleep_pad) {
                touch_pad_sleep_channel_read_smooth(&touch_value);
                touch_pad_sleep_set_threshold(proximity.meas_num * touch_value * (1 + TOUCH_INTR_THRESHOLD));
                ESP_LOGI(TAG, "Sleep pad [%d] base %d, thresh %d", touch_list[i], touch_value,
                         (uint32_t)(proximity.meas_num * touch_value * (1 + TOUCH_INTR_THRESHOLD)));
            } else if (touch_list[i] == proximity.select_pad[0] ||
                       touch_list[i] == proximity.select_pad[1] ||
                       touch_list[i] == proximity.select_pad[2]) {
                // TEST_ESP_OK( touch_pad_filter_read_baseline(touch_list[i], &touch_value) );
                touch_pad_sleep_channel_read_smooth(&touch_value);
        ...
        /* Sleep channel setting */
        TEST_ESP_OK( touch_pad_sleep_channel_read_smooth(&touch_value) );
        TEST_ESP_OK( touch_pad_sleep_set_threshold(touch_value * TOUCH_INTR_THRESHOLD) );
        vTaskDelay(50 * SYS_DELAY_TIME_MOM / portTICK_PERIOD_MS);
    }

    TEST_ESP_OK( touch_pad_sleep_channel_read_smooth(&ret_val) );
    ...
    while (test_cnt--) {
        ...
        printf_touch_hw_read("push");
        if (is_proximity) {
            TEST_ESP_OK( touch_pad_sleep_channel_read_smooth(&smooth) );
            TEST_ESP_OK( touch_pad_sleep_channel_read_baseline(&touch_value) );
            TEST_ESP_OK( touch_pad_proximity_data_get(sleep_pad, &measure_out) );
            TEST_ESP_OK( touch_pad_sleep_channel_read_proximity_cnt(&proximity_cnt) );
            TEST_ESP_OK( touch_pad_sleep_channel_read_debounce(&dbc) );
            TEST_ESP_OK( touch_pad_sleep_get_threshold(&touch_thres) );
            printf("touch slp smooth %d, base %d, proxi %d cnt %d dbc %d thres%d status 0x%x\n",
                   smooth, touch_value, measure_out, proximity_cnt, dbc,
        ...
        if (is_proximity) {
            ...
            TEST_ESP_OK( touch_pad_sleep_channel_read_proximity_cnt(&proximity_cnt) );
            TEST_ESP_OK( touch_pad_sleep_channel_read_debounce(&dbc) );
            printf("touch slp smooth %d, base %d, proxi %d cnt %d dbc %d status 0x%x\n",
                   smooth, touch_value, measure_out, proximity_cnt, dbc, touch_pad_get_status());
        }
    }
...
esp_err_t test_touch_sleep_pad_interrupt_wakeup_deep_sleep(touch_pad_t sleep_pad)
{
    ...
    printf_touch_hw_read("push");
    TEST_ESP_OK( touch_pad_sleep_channel_read_smooth(&smooth) );
    TEST_ESP_OK( touch_pad_sleep_channel_read_data(&raw) );
    TEST_ESP_OK( touch_pad_sleep_channel_read_baseline(&touch_value) );
...
// TEST_CASE("Touch Sensor sleep pad wakeup deep sleep test", "[touch]")
void test_touch_sleep_wakeup(void)
{

...
CONFIGS = [
    pytest.param(
        'default',
        marks=[pytest.mark.supported_targets, pytest.mark.temp_skip_ci(targets=['esp32h2'], reason='test failed')],
    ),
    pytest.param(
        'freertos_options',
        marks=[pytest.mark.supported_targets, pytest.mark.temp_skip_ci(targets=['esp32h2'], reason='test failed')],
    ...
    pytest.param(
        ...
        marks=[
            ...
            pytest.mark.temp_skip_ci(targets=['esp32h2', 'esp32p4'], reason='test failed/TBD IDF-8113'),
        ],
    ),
...
@idf_parametrize(
    ...
    [
        ('default', 'supported_targets', (pytest.mark.temp_skip_ci(targets=['esp32h2'], reason='test failed'),)),
        (
            'freertos_options',
            'supported_targets',

...
TEST_CASE("FreeRTOS Event Groups", "[freertos]")
{
    ...
    for (int c = 0; c < NUM_TASKS; c++) {
        xTaskCreatePinnedToCore(task_event_group_call_response, "tsk_call_resp", 4096, (void *)c, configMAX_PRIORITIES - 1, &task_handles[c], c % configNUM_CORES);
    }
...
TEST_CASE("FreeRTOS Event Group Sync", "[freertos]")
{
    ...
    for (int c = 0; c < NUM_TASKS; c++) {
        xTaskCreatePinnedToCore(task_test_sync, "task_test_sync", 4096, (void *)c, configMAX_PRIORITIES - 1, NULL, c % configNUM_CORES);
    }

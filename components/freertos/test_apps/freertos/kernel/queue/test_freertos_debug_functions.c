...
#define NO_OF_QUEUES_PER_CORE  ((int)((CONFIG_FREERTOS_QUEUE_REGISTRY_SIZE - 3)/configNUM_CORES))     //Save space for some preallocated tasks
#define NO_OF_QUEUES_TOTAL    (NO_OF_QUEUES_PER_CORE * configNUM_CORES)
#define QUEUE_NAME_MAX_LENGTH    30

static SemaphoreHandle_t start_sem[configNUM_CORES];
...
TEST_CASE("Test FreeRTOS Queue Registry", "[freertos]")
{
    ...
    done_sem = xSemaphoreCreateCounting(configNUM_CORES, 0);

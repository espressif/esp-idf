...
#define TEST_FREERTOS_NUM_CORES 1
#else
#define TEST_FREERTOS_NUM_CORES SOC_CPU_CORES_NUM
#endif
...
#endif // TEST_FREERTOS_NUM_CORES > 1
...
TEST_CASE("IDF additions: xTaskGetCoreID for pinned tasks", "[freertos][idf_additions]")
{
    ...
    core_id_query_ctx_t ctx[TEST_FREERTOS_NUM_CORES];

    for (int core = 0; core < TEST_FREERTOS_NUM_CORES; core++) {
...
#if (TEST_FREERTOS_NUM_CORES > 1) && !CONFIG_FREERTOS_SMP
...
#endif /* TEST_FREERTOS_NUM_CORES > 1 && !CONFIG_FREERTOS_SMP */

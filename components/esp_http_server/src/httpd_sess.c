...
typedef enum {
      task_none = 0
    , task_init            // Init session
    , task_get_active      // Get active session (fd!=-1)
    , task_get_free        // Get free session slot (fd<0)
    , task_find_fd         // Find session with specific fd
    , task_set_descriptor  // Set descriptor
    , task_delete_invalid  // Delete invalid session
    , task_find_lowest_lru // Find session with lowest lru
	, task_close           // Close session
} task_t;
...
void httpd_sess_enum(struct httpd_data * hd, httpd_session_enum_function enumFunction, void * context)
{
    if ((!hd)
     || (!hd->hd_sd)
...
static int EnumFunction(struct sock_db * session, void * context)
{
    if ((!session)
     || (!context)) {
        return 0;
...
{
	if ((!hd)
     || (hd->hd_sd_active_count==hd->config.max_open_sockets)) {
		return NULL;
	}
    enum_context_t context = {.task = task_get_free};
    httpd_sess_enum(hd, EnumFunction, &context);
...
struct sock_db * httpd_sess_get(struct httpd_data *hd, int sockfd)
{
    ...
    if ((hd->hd_req_aux.sd)
     && (hd->hd_req_aux.sd->fd == sockfd)) {
         return hd->hd_req_aux.sd;
    }

    enum_context_t context = {.task = task_find_fd, .fd = sockfd};
    httpd_sess_enum(hd, EnumFunction, &context);
    return context.session;
}
...
{
    enum_context_t context = {.task = task_set_descriptor, .max_fd = -1, .fdset = fdset};
    httpd_sess_enum(hd, EnumFunction, &context);
    if (maxfd) {
...
{
    enum_context_t context = {.task = task_delete_invalid, .hd = hd};
    httpd_sess_enum(hd, EnumFunction, &context);
}
...
{
    enum_context_t context = {.task = task_init};
    httpd_sess_enum(hd, EnumFunction, &context);
}
...
{
    enum_context_t context = {.task = task_find_lowest_lru, .lru_counter = UINT64_MAX, .fd = -1};
    httpd_sess_enum(hd, EnumFunction, &context);
    if (!context.session) {
        return ESP_OK;
    }
...
{
	enum_context_t context = {.task = task_close,.hd=hd};
	httpd_sess_enum(hd, EnumFunction, &context);
}

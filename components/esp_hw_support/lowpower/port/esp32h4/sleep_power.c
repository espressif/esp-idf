...
esp_err_t sleep_power_system_retention_init(void *arg)
{
    const static sleep_retention_entries_config_t power_regs_retention[] = {
        /* During the modem-to-active transition, the MODEM lock keeps the xtalx2 xpd status unchanged.
        * Therefore, if xtalx2 is disabled in modem state, we need to set xtalx2 tie-high to enable it in active state. */
    ...
    ESP_RETURN_ON_ERROR(err, TAG, "failed to allocate memory for system power retention");
    ESP_LOGI(TAG, "System power sleep retention initialization");
    return ESP_OK;
}

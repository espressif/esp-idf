...
#define ALIGN_UP_TO_MMU_PAGE_SIZE(addr)    ALIGN_UP(addr, SOC_MMU_PAGE_SIZE)
...
static void esp_cpu_configure_valid_regions(void)
{
    ...
    PMP_ENTRY_SET(8, (uint32_t)(&_instruction_reserved_end), PMP_TOR | RX);
    PMP_ENTRY_SET(9, page_aligned_irom_resv_end, PMP_TOR | RW);
    PMP_ENTRY_SET(10, ALIGN_UP(page_aligned_irom_resv_end + available_psram_heap, SOC_CPU_PMP_REGION_GRANULARITY), PMP_TOR | RW);
    ...
    const uint32_t page_aligned_drom_resv_end = ALIGN_UP_TO_MMU_PAGE_SIZE((uint32_t)(&_rodata_reserved_end));
    PMP_ENTRY_SET(9, ALIGN_UP(page_aligned_drom_resv_end + available_psram_heap, SOC_CPU_PMP_REGION_GRANULARITY), PMP_TOR | RW);

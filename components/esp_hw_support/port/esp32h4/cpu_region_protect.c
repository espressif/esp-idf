...
{
    const unsigned PMA_NONE                            = CONDITIONAL_PMA_L | PMA_EN;
...
void esp_cpu_configure_region_protection(void)
{
    ...
    if (esp_cpu_dbgr_is_attached()) {
        ...
        PMP_ENTRY_SET(4, SOC_HP_RAM_HIGH, PMP_TOR | RWX);
        ...
        PMP_ENTRY_SET(5, SOC_HP_RAM_HIGH, PMP_TOR | RW);
#else
        PMP_ENTRY_SET(3, SOC_IRAM_LOW, CONDITIONAL_NONE);
        PMP_ENTRY_SET(4, SOC_HP_RAM_HIGH, PMP_TOR | CONDITIONAL_RWX);
        _Static_assert(SOC_IRAM_LOW < SOC_IRAM_HIGH, "Invalid RAM region");
#endif
    }

...
esp_err_t dma2d_set_desc_addr(dma2d_channel_handle_t dma2d_chan, intptr_t desc_base_addr)
{
    ...
    ESP_GOTO_ON_FALSE_ISR(!efuse_hal_flash_encryption_enabled() || esp_ptr_internal((void *)desc_base_addr), ESP_ERR_INVALID_ARG, err, TAG, "invalid argument");
...
esp_err_t dma2d_set_transfer_ability(dma2d_channel_handle_t dma2d_chan, const dma2d_transfer_ability_t *ability)
{
    ...
    if (efuse_hal_flash_encryption_enabled() && ability->access_ext_mem) {
...
esp_err_t dma2d_dequeue(dma2d_pool_handle_t dma2d_pool, dma2d_trans_t *trans)
{
    ...
    bool found = false;
    portENTER_CRITICAL(&dma2d_group->spinlock);
    ...
    portEXIT_CRITICAL(&dma2d_group->spinlock);

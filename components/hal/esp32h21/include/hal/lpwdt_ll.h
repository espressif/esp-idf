...
{
    return (hw->wdt_config0.wdt_wdt_en) ? true : false;
}
...
FORCE_INLINE_ATTR void lpwdt_ll_config_stage(lp_wdt_dev_t *hw, wdt_stage_t stage, uint32_t timeout_ticks, wdt_stage_action_t behavior)
{
    switch (stage) {
    case WDT_STAGE0:
        hw->wdt_config0.wdt_wdt_stg0 = behavior;
        //Account of implicty multiplier applied to stage 0 timeout tick config value
        hw->wdt_config1.val = timeout_ticks >> (1 + REG_GET_FIELD(EFUSE_RD_REPEAT_DATA1_REG, EFUSE_WDT_DELAY_SEL));
        break;
    case WDT_STAGE1:
        hw->wdt_config0.wdt_wdt_stg1 = behavior;
        hw->wdt_config2.val = timeout_ticks;
        break;
    case WDT_STAGE2:
        hw->wdt_config0.wdt_wdt_stg2 = behavior;
        hw->wdt_config3.val = timeout_ticks;
        break;
    case WDT_STAGE3:
        hw->wdt_config0.wdt_wdt_stg3 = behavior;
        hw->wdt_config4.val = timeout_ticks;
        break;
    default:
        abort();
    }
...
FORCE_INLINE_ATTR void lpwdt_ll_disable_stage(lp_wdt_dev_t *hw, wdt_stage_t stage)
{
    switch (stage) {
    case WDT_STAGE0:
        hw->wdt_config0.wdt_wdt_stg0 = WDT_STAGE_ACTION_OFF;
        break;
    case WDT_STAGE1:
        hw->wdt_config0.wdt_wdt_stg1 = WDT_STAGE_ACTION_OFF;
        break;
    case WDT_STAGE2:
        hw->wdt_config0.wdt_wdt_stg2 = WDT_STAGE_ACTION_OFF;
        break;
    case WDT_STAGE3:
        hw->wdt_config0.wdt_wdt_stg3 = WDT_STAGE_ACTION_OFF;
        break;
    default:
        abort();
    }
...
{
    hw->wdt_config0.wdt_wdt_cpu_reset_length = length;
}
...
{
    hw->wdt_config0.wdt_wdt_sys_reset_length = length;
}
...
{
    hw->wdt_config0.wdt_wdt_flashboot_mod_en = (enable) ? 1 : 0;
}
...
{
    hw->wdt_config0.wdt_wdt_procpu_reset_en = (enable) ? 1 : 0;
}
...
{
    hw->wdt_config0.wdt_wdt_appcpu_reset_en = (enable) ? 1 : 0;
}
...
{
    hw->wdt_config0.wdt_wdt_pause_in_slp = (enable) ? 1 : 0;
}
...
{
    hw->wdt_config5.wdt_chip_reset_en = (enable) ? 1 : 0;
}

/**
 * @brief No register for setting reset width on H21, we keep an empty function to
...
{
    hw->wdt_feed.wdt_rtc_wdt_feed = 1;
}
...
{
    hw->wdt_wprotect.val = LP_WDT_WKEY_VALUE;
}
...
{
    hw->wdt_int_ena.wdt_super_wdt_int_ena = (enable) ? 1 : 0;
}
...
{
    return (hw->wdt_int_st.wdt_lp_wdt_int_st) ? true : false;
}
...
{
    hw->wdt_int_clr.wdt_lp_wdt_int_clr = 1;
}

...
/* Combined size of all TX segments in one transaction (H4 type byte + one ACL payload). */
#define HCI_TX_MAX_SIZE                     (DEFAULT_BT_LE_ACL_BUF_SIZE + 1)
/* One HCI packet budget; used only to size the RX rings, not a mempool block anymore. */
 ...
 */
#define HCI_UHCI_RX_DESC_MEM                (CONFIG_BT_LE_HCI_TRANS_RX_MEM_NUM * HCI_RX_DMA_RING_SIZE)

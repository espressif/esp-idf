...
{
    esp_mesh_health_client_cb_t btc_mesh_cb = (esp_mesh_health_client_cb_t)btc_profile_cb_get(BTC_PID_HEALTH_CLIENT);
    if (btc_mesh_cb) {
...
{
    esp_mesh_health_server_cb_t btc_mesh_cb = (esp_mesh_health_server_cb_t)btc_profile_cb_get(BTC_PID_HEALTH_SERVER);
    if (btc_mesh_cb) {
        btc_mesh_cb(event, param);
    }
}

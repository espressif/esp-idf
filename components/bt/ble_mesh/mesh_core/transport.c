                       ...
                       const struct bt_mesh_send_cb *cb, void *cb_data)
{
    ...
    if (tx->ctx->app_idx == BT_MESH_KEY_DEV) {
        ...
        if (!key) {
            BT_ERR("%s: get device key fail", __func__);
            return -EINVAL;
        }
...
static bool is_replay(struct bt_mesh_net_rx *rx)
{
    ...
    for (i = 0; i < ARRAY_SIZE(bt_mesh.rpl); i++) {
        ...
        if (rpl->src == rx->ctx.addr) {
            ...
            /* We add 10 here for fixing the bug of Silicon Lab Android App 1.1.0 when reconnection will cause the sequence number to recount from 0. */
            if ((!rx->old_iv && rpl->old_iv) ||
                    ...
                    struct net_buf_simple *buf)
{
    ...
    if (!AKF(&hdr)) {
        ...
        BT_WARN("%s: no dev_key found", __func__);
        return -ENODEV;
    }
    ...
    for (i = 0; i < array_size; i++) {
        ...
        if (!key) {
            BT_DBG("%s: get NULL app_key", __func__);
            continue;
        }

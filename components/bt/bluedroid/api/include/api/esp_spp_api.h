...
#define ESP_SPP_ANY_SCN                 0           /*!< SPP server accepts Clients regardless of SCN */
/**
 * @brief SPP callback function events
 */
typedef enum {
    ESP_SPP_INIT_EVT                    = 0,                /*!< When SPP is initiated, the event comes */
...
typedef union {
    ...
    struct spp_srv_open_evt_param {
        ...
        int                 fd;             /*!< The file descriptor only for ESP_SPP_MODE_VFS*/
        uint8_t             scn;            /*!< The server channel number */
        esp_bd_addr_t       rem_bda;        /*!< The peer address */
    ...
    struct spp_cong_evt_param {
        esp_spp_status_t    status;         /*!< status */
        uint32_t            handle;         /*!< The connection handle */
        bool                cong;           /*!< TRUE, congested. FALSE, not congested */

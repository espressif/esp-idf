...
void BTM_BleUpdateAdvFilterPolicy(tBTM_BLE_AFP adv_policy)
{
    ...
    if (p_cb->afp != adv_policy)
    {
        ...
        btsnd_hcic_ble_write_adv_params((UINT16)(p_cb->adv_interval_min ? p_cb->adv_interval_min : BTM_BLE_GAP_ADV_SLOW_INT),
                                        (UINT16)(p_cb->adv_interval_max ? p_cb->adv_interval_max : BTM_BLE_GAP_ADV_SLOW_INT),
                                        p_cb->evt_type,
                                        p_cb->adv_addr_type,
                                        init_addr_type,
                                        p_addr_ptr,
...
extern void BTM_BleReadControllerFeatures(tBTM_BLE_CTRL_FEATURES_CBACK *p_vsc_cback)
{
    ...
    if (BTM_VendorSpecificCommand(HCI_BLE_VENDOR_CAP_OCF,
                                  0,
                                  NULL,
                                  btm_ble_vendor_capability_vsc_cmpl_cback) != BTM_CMD_STARTED)
    {
...
BOOLEAN BTM_BleConfigPrivacy(BOOLEAN privacy_mode, tBTM_SET_LOCAL_PRIVACY_CBACK *set_local_privacy_cback)
{
    ...
    if (!privacy_mode)
    { /* if privacy disabled, always use public address */
...
{
    return btm_cb.cmn_ble_vsc_cb.adv_inst_max < BTM_BLE_MULTI_ADV_MAX ? btm_cb.cmn_ble_vsc_cb.adv_inst_max : BTM_BLE_MULTI_ADV_MAX;
}
                                             ...
                                             tBLE_ADDR_TYPE *p_own_addr_type)
{
    ...
    evt_type = (p_cb->connectable_mode == BTM_BLE_NON_CONNECTABLE) ? ((p_cb->scan_rsp) ? BTM_BLE_DISCOVER_EVT : BTM_BLE_NON_CONNECT_EVT)
                                                                   : BTM_BLE_CONNECT_EVT;
...
UINT8 *BTM_CheckAdvData(UINT8 *p_adv, UINT8 type, UINT8 *p_length)
{
    ...
    while (length && (p - p_adv <= BTM_BLE_CACHE_ADV_DATA_MAX))
    {
        ...
        while ((*p == 0) && (p - p_adv <= BTM_BLE_ADV_DATA_LEN_MAX))
        {
            p++;
        }
        STREAM_TO_UINT8(length, p);
...
tBTM_STATUS btm_ble_set_discoverability(UINT16 combined_mode)
{
    ...
    if (new_mode == BTM_BLE_ADV_ENABLE)
    {
        ...
        if (evt_type != p_cb->evt_type || p_cb->adv_addr_type != own_addr_type || !p_cb->fast_adv_on)
        {
            btm_ble_stop_adv();

...
/**
 * @brief Bluetooth Low Energy Advertising Data Flag bit definition used for advertising data flag.
 */
#define ESP_BLE_ADV_FLAG_LIMIT_DISC         (0x01 << 0)     /*!< ESP Bluetooth Low Energy Advertising Flag Limited Discoverable Mode */
#define ESP_BLE_ADV_FLAG_GEN_DISC           (0x01 << 1)     /*!< ESP Bluetooth Low Energy Advertising Flag General Discoverable Mode */
#define ESP_BLE_ADV_FLAG_BREDR_NOT_SPT      (0x01 << 2)     /*!< ESP Bluetooth Low Energy Advertising Flag Basic Rate/Enhanced Data Rate Not Supported */
#define ESP_BLE_ADV_FLAG_DMT_CONTROLLER_SPT (0x01 << 3)     /*!< ESP Bluetooth Low Energy Advertising Flag Dual-Mode Controller Supported */
#define ESP_BLE_ADV_FLAG_DMT_HOST_SPT       (0x01 << 4)     /*!< ESP Bluetooth Low Energy Advertising Flag Dual-Mode Host Supported */
#define ESP_BLE_ADV_FLAG_NON_LIMIT_DISC     (0x00 )         /*!< ESP Bluetooth Low Energy Advertising Flag Non-Limited Discoverable Mode */


/**
 * @brief Relate to BTM_LE_KEY_xxx in stack/btm_api.h.
 * These definitions pertain to the ESP32 Bluetooth Low Energy (BLE) stack.
 * Describing different types of keys used in BLE communication and security protocols.
 */
#define ESP_LE_KEY_NONE      0           /*!< No Link Encryption Key is used. */
#define ESP_LE_KEY_PENC      (1 << 0)    /*!< Pseudo-Random Number Generator (PRNG) Encryption Key. */
#define ESP_LE_KEY_PID       (1 << 1)    /*!< Identity Resolving Key (IRK) for Privacy. */
#define ESP_LE_KEY_PCSRK     (1 << 2)    /*!< Privacy-Enhanced Connection Signature Resolving Key (CSRK). */
#define ESP_LE_KEY_PLK       (1 << 3)    /*!< Long-Term Key (LTK) for Pairing. */
#define ESP_LE_KEY_LLK       (ESP_LE_KEY_PLK << 4)      /*!< Long-Term Key (LTK) for Legacy. */
#define ESP_LE_KEY_LENC      (ESP_LE_KEY_PENC << 4)     /*!< Long-Term Key (LTK) for Link Encryption. */
#define ESP_LE_KEY_LID       (ESP_LE_KEY_PID << 4)      /*!< Link Identity Key (LID). */
#define ESP_LE_KEY_LCSRK     (ESP_LE_KEY_PCSRK << 4)    /*!< Link Connection Signature Resolving Key (CSRK). */

typedef uint8_t esp_ble_key_type_t;  /*!< Type definition for BLE key types. */
...
/**
 * @brief Relate to BTM_LE_AUTH_xxx in stack/btm_api.h.
 * These macros are used in defining the BLE authentication requirements in the ESP32 Bluetooth Low Energy (BLE) stack.
 */
#define ESP_LE_AUTH_NO_BOND             0x00        /*!< No bonding required. */
#define ESP_LE_AUTH_BOND                0x01        /*!< Device is bonded with the peer. */
#define ESP_LE_AUTH_REQ_MITM            (1 << 2)    /*!< Man-in-the-middle attack required. */
#define ESP_LE_AUTH_REQ_BOND_MITM       (ESP_LE_AUTH_BOND | ESP_LE_AUTH_REQ_MITM) /*!< Bonding with Man-in-the-middle protection. */
#define ESP_LE_AUTH_REQ_SC_ONLY         (1 << 3)    /*!< Secure connection (SC) only. */
#define ESP_LE_AUTH_REQ_SC_BOND         (ESP_LE_AUTH_BOND | ESP_LE_AUTH_REQ_SC_ONLY) /*!< Bonding with Secure connection. */
#define ESP_LE_AUTH_REQ_SC_MITM         (ESP_LE_AUTH_REQ_MITM | ESP_LE_AUTH_REQ_SC_ONLY) /*!< Secure connection with MITM protection. */
#define ESP_LE_AUTH_REQ_SC_MITM_BOND    (ESP_LE_AUTH_REQ_MITM | ESP_LE_AUTH_REQ_SC_ONLY | ESP_LE_AUTH_BOND) /*!< Secure connection with MITM and Bonding. */
typedef uint8_t esp_ble_auth_req_t;    /*!< Type definition for bluetooth low energy authentication requirements. */

#define ESP_BLE_ONLY_ACCEPT_SPECIFIED_AUTH_DISABLE    0    /*!< Authentication disabled. */
#define ESP_BLE_ONLY_ACCEPT_SPECIFIED_AUTH_ENABLE     1    /*!< Authentication enabled. */

#define ESP_BLE_OOB_DISABLE    0    /*!< Out-of-Band (OOB) data authentication disabled. */
#define ESP_BLE_OOB_ENABLE     1    /*!< Out-of-Band data authentication enabled. */

/**
 * @brief Relate to BTM_IO_CAP_xxx in stack/btm_api.h.
 * These macros define the various input/output capabilities for devices in the ESP32 framework, particularly for Bluetooth operations.
 */
...
#define    BLE_DTM_PKT_PAYLOAD_0x00   0x00       /*!< Pseudo-Random Binary Sequence 9 (PRBS9) sequence ‘11111111100000111101...’ (in transmission order) as described in [Vol 6] Part F, Section 4.1.5 */
...
#define    BLE_DTM_PKT_PAYLOAD_0x03   0x03       /*!< PRBS 15 sequence as described in [Vol 6] Part F, Section 4.1.5 */
...
typedef uint8_t esp_ble_dtm_pkt_payload_t;       /*!< ESP32 Bluetooth Low Energy Direct Test Mode Packet Payload Type */


/**
 * @brief Generic access profile bluetooth low energy callback event type
 */
typedef enum {
//@{
...
#define ESP_GAP_BLE_CHANNELS_LEN     5                           /*!< Channel length includes three broadcast channels and two reserved channels. */
typedef uint8_t esp_gap_ble_channels[ESP_GAP_BLE_CHANNELS_LEN];  /*!< Array type representing BLE AFH channel map. */

/**
 * @brief This is the old name, just for backwards compatibility.
 */
...
/**
 * @brief Relate to BTM_BLE_AD_TYPE_xxx in stack/btm_ble_api.h.
 ...
 */
typedef enum {
ADV_TYPE_IND             = 0x00,     /*!< Indicates Undirected Advertising. */
ADV_TYPE_DIRECT_IND_HIGH = 0x01,     /*!< High Duty Cycle Directed Advertising.
                                        Advertising channel index shall be less than or equal to 3.75 ms. 
                                        The Link Layer shall exit the Advertising state no later than 1.28 s after the
                                        Advertising state was entered. */
ADV_TYPE_SCAN_IND        = 0x02,     /*!< Scannable Undirected Advertising. */
ADV_TYPE_NONCONN_IND     = 0x03,     /*!< Non-Connectable Undirected Advertising. */
ADV_TYPE_DIRECT_IND_LOW  = 0x04      /*!< Low Duty Cycle Directed Advertising.
                                        Advertising event shall be less than or equal to 10 ms. */
} esp_ble_adv_type_t;

/**
 * @brief Advertising channel mask
 * At least use one of the channels below.
 */
typedef enum {
    ADV_CHNL_37     = 0x01,     /*!< Indicates the channel used for broadcasting 37 channels.*/
    ADV_CHNL_38     = 0x02,     /*!< Indicates the channel used for broadcasting 38 channels.*/
    ADV_CHNL_39     = 0x04,     /*!< Indicates the channel used for broadcasting 39 channels.*/
    ADV_CHNL_ALL    = 0x07,     /*!< Indicates that all broadcast channels are used.*/
...
typedef enum {
    ...
    ADV_FILTER_ALLOW_SCAN_WLST_CON_ANY,      /*！< Allow scan requests from White List devices only and connection requests from anyone. */
    ADV_FILTER_ALLOW_SCAN_ANY_CON_WLST,      /*！< Allow scan requests from anyone and connection requests from White List devices only. */
    ADV_FILTER_ALLOW_SCAN_WLST_CON_WLST,     /*！< Allow scan and connection requests from White List devices only. */
} esp_ble_adv_filter_t;


/**
 * @brief relate to BTA_DM_BLE_SEC_xxx in bta/bta_api.h
 */
typedef enum {
    ESP_BLE_SEC_ENCRYPT = 1,            /*!< relate to BTA_DM_BLE_SEC_ENCRYPT in bta/bta_api.h. If the device has already
                                           ...
                                           Else if the device hasn't bonded, the stack will used the default authentication request (the same as NO MITM authentication request)
    ...
    ESP_BLE_SEC_ENCRYPT_NO_MITM,        /*!< relate to BTA_DM_BLE_SEC_ENCRYPT_NO_MITM in bta/bta_api.h. If the device has been already
                                           bonded, the stack will check the LTK (Long Term Key) Whether the authentication request has been met, and if met, use the Long Term Key
                                           ...
                                           using the MITM authentication in the esp_ble_gap_set_security_param function set by the user. */
...
/**
 * @brief These annotations clarify the purpose of each enum member related to DTM (Direct Test Mode) testing in bluetooth low energy.
 */
typedef enum {
    DTM_TX_START_EVT  = 0x00,   /*!< When the Direct Test Mode TX test starts, this event is generated. */
    DTM_RX_START_EVT,           /*!< When the Direct Test Mode RX test starts, this event is generated. */
    DTM_TEST_STOP_EVT,          /*!< When the Direct Test Mode test ends, this event is generated. */
 ...
 */
typedef struct {
    uint16_t opcode;                /*!< Vendor hci command opcode. Value range: 0x0001-0xFFFF */
...
/**
* @brief Advertising data content, according to "Supplement to the Bluetooth Core Specification"
*/
typedef struct {
...
/**
* @brief Bluetooth low energy scan type
*/
typedef enum {
...
/**
* @brief Bluetooth low energy scan filter type
*/
typedef enum {
...
/**
* @brief Bluetooth low energy scan duplicate type
*/
typedef enum {
...
/**
* @brief Bluetooth low energy scan parameters
*/
typedef struct {
    esp_ble_scan_type_t     scan_type;              /*!< Scan type, range:0x0 or 0x1*/
    esp_ble_addr_type_t     own_addr_type;          /*!< Owner address type, range:0x00 - 0x03, see in api\esp_bt_defs.h*/
    esp_ble_scan_filter_t   scan_filter_policy;     /*!< Scan filter policy, range:0x0 - 0x3*/
    uint16_t                scan_interval;          /*!< Scan interval. This is the time interval between when the controller 
                                                      started the last low energy scan and when it starts the next low energy scan.
                                                      Range: 0x0004 to 0x4000 Default: 0x0010 (10 ms)
                                                      Time = N * 0.625 msec
                                                      Time Range: 2.5 msec to 10.24 seconds*/
    uint16_t                scan_window;            /*!< Scan window. The duration of the low energy scan. LE_Scan_Window
...
typedef struct {
    ...
    uint16_t             timeout;                   /*!< Supervision timeout for the low energy Link. Range: 0x000A to 0x0C80.
...
/**
* @brief bluetooth low energy pkt date length keys
*/
typedef struct
{
    uint16_t rx_len;                   /*!< packet rx data length value */
    uint16_t tx_len;                   /*!< packet tx data length value */
} esp_ble_pkt_data_length_params_t;

/**
* @brief bluetooth low energy encryption keys
* The key type.
*/
typedef struct
{
    ...
    uint16_t             ediv;         /*!< The value of the encryption diversifier. */
...
/**
* @brief  bluetooth low energy connection signature resolving keys
* The Privacy-Enhanced connection signature resolving key type. 
*/
typedef struct
{
    uint32_t            counter;      /*!< The counter. */
    esp_bt_octet16_t    csrk;         /*!< The connection signature resolving key. */
    uint8_t             sec_level;    /*!< The security level. */
} esp_ble_pcsrk_keys_t;              

/**
* @brief  bluetooth low energy pairing identification keys
*/
typedef struct
{
    esp_bt_octet16_t          irk;           /*!< The identity resolving key value. Typically a 128-bit (16-byte) random number used 
                                                for device identity resolution and privacy protection. */
    esp_ble_addr_type_t       addr_type;     /*!< The address type. */
    esp_bd_addr_t             static_addr;   /*!< The static address. */
} esp_ble_pid_keys_t;

/**
* @brief  bluetooth low energy encryption reproduction keys
*/
typedef struct
{
    esp_bt_octet16_t  ltk;                  /*!< The long term key. */
...
* Bluetooth low energy key notify type. 
*/
typedef struct
{
    esp_bd_addr_t  bd_addr;        /*!< peer address */
    uint32_t       passkey;        /*!< The numeric value for comparison. If just_works, do not show this number to uer interface. */
} esp_ble_sec_key_notif_t;

/**
* @brief  Structure of the security request
* Bluetooth low energy security request type.
*/
typedef struct
{
    esp_bd_addr_t  bd_addr;        /*!< peer address */
...
typedef union
{
    ...
    esp_ble_pcsrk_keys_t  pcsrk_key;      /*!< received peer device Secure Resolving Key */
...
typedef struct
{
    ...
    esp_ble_pid_keys_t    pid_key;        /*!< peer device ID key */
} esp_ble_bond_key_info_t;                /*!< bluetooth low energy bond key information value type */
...
/**
* @brief  structure type of the bluetooth low energy local id keys value
* the structure of the bluetooth low energy local id keys value type
*/
typedef struct {
    esp_bt_octet16_t       ir;                  /*!< the 16 bits of the Identity Root value */
    esp_bt_octet16_t       irk;                 /*!< the 16 bits of the Identity Root key value */
    esp_bt_octet16_t       dhk;                 /*!< the 16 bits of the Diffie-Hellman key value */
...
/**
  ...
  * * The bluetooth low energy authentication complete callback type.
  */
typedef struct
{
    ...
    uint8_t               fail_reason;           /*!< The Host Controller Interface reason/error code for when success=FALSE. */
...
/**
  * @brief union associated with bluetooth low energy security
  * bluetooth low energy security type
  */
typedef union
{
    esp_ble_sec_key_notif_t    key_notif;      /*!< passkey notification */
    esp_ble_sec_req_t          ble_req;        /*!< bluetooth low energy Security Manager Protocol related request */
    esp_ble_key_t              ble_key;        /*!< bluetooth low energy Security Manager Protocol keys used when pairing */
    esp_ble_local_id_keys_t    ble_id_keys;    /*!< bluetooth low energy Identity Root event */
    esp_ble_local_oob_data_t   oob_data;       /*!< bluetooth low energy Security Manager Protocol secure connection Out-of-Band data */
...
/**
 * @brief Sub Event of ESP_GAP_BLE_SCAN_RESULT_EVT
 */
typedef enum {
...
/**
 * @brief Bluetooth Low Energy scan result event type, to indicate the
...
typedef enum {
    ...
    ESP_BLE_DUPLICATE_EXCEPTIONAL_LIST_REMOVE,        /*!< Remove device information from duplicate scan exceptional list. */
...
#if (BLE_42_FEATURE_SUPPORT == TRUE)
typedef enum {
    ESP_BLE_DUPLICATE_SCAN_EXCEPTIONAL_INFO_ADV_ADDR       = 0,  /*!< Bluetooth Low Energy advertising address , device info will be added into ESP_BLE_DUPLICATE_SCAN_EXCEPTIONAL_ADDR_LIST. */
    ESP_BLE_DUPLICATE_SCAN_EXCEPTIONAL_INFO_MESH_LINK_ID,        /*!< Bluetooth Low Energy mesh link ID, it is for Bluetooth Low Energy mesh, device info will be added into ESP_BLE_DUPLICATE_SCAN_EXCEPTIONAL_MESH_LINK_ID_LIST. */
    ESP_BLE_DUPLICATE_SCAN_EXCEPTIONAL_INFO_MESH_BEACON_TYPE,    /*!< Bluetooth Low Energy mesh beacon Advertising Data type, the format is | Len | 0x2B | Beacon Type | Beacon Data | */
    ESP_BLE_DUPLICATE_SCAN_EXCEPTIONAL_INFO_MESH_PROV_SRV_ADV,   /*!< Bluetooth Low Energy mesh provisioning service uuid, the format is | 0x02 | 0x01 | flags | 0x03 | 0x03 | 0x1827 | .... |` */
    ESP_BLE_DUPLICATE_SCAN_EXCEPTIONAL_INFO_MESH_PROXY_SRV_ADV,  /*!< Bluetooth Low Energy mesh advertising with proxy service uuid, the format is | 0x02 | 0x01 | flags | 0x03 | 0x03 | 0x1828 | .... |` */
    ESP_BLE_DUPLICATE_SCAN_EXCEPTIONAL_INFO_MESH_PROXY_SOLIC_ADV,  /*!< Bluetooth Low Energy mesh advertising with proxy service uuid, the format is | 0x02 | 0x01 | flags | 0x03 | 0x03 | 0x1859 | .... |` */
    ESP_BLE_DUPLICATE_SCAN_EXCEPTIONAL_INFO_MESH_URI_ADV,        /*!< Bluetooth Low Energy mesh Uniform Resource Identifier advertising, the format is ...| Len | 0x24 | data |... */
} esp_ble_duplicate_exceptional_info_type_t;

typedef enum {
    ESP_BLE_DUPLICATE_SCAN_EXCEPTIONAL_ADDR_LIST                  = BLE_BIT(0),             /*!< duplicate scan exceptional address list */
    ...
    ESP_BLE_DUPLICATE_SCAN_EXCEPTIONAL_MESH_PROV_SRV_ADV_LIST     = BLE_BIT(3),             /*!< duplicate scan exceptional mesh advertising with provisioning service uuid */
    ESP_BLE_DUPLICATE_SCAN_EXCEPTIONAL_MESH_PROXY_SRV_ADV_LIST    = BLE_BIT(4),             /*!< duplicate scan exceptional mesh advertising with proxy service uuid */
    ESP_BLE_DUPLICATE_SCAN_EXCEPTIONAL_MESH_PROXY_SOLIC_ADV_LIST  = BLE_BIT(5),             /*!< duplicate scan exceptional mesh advertising with proxy solicitation Protocol Data Unit uuid */
    ESP_BLE_DUPLICATE_SCAN_EXCEPTIONAL_MESH_URI_ADV_LIST          = BLE_BIT(6),             /*!< duplicate scan exceptional Uniform Resource Identifier list */
...
#define ESP_BLE_GAP_SET_EXT_ADV_PROP_LEGACY                               (1 << 4) /*!< Use legacy advertising Protocol Data Units. */
#define ESP_BLE_GAP_SET_EXT_ADV_PROP_ANON_ADV                             (1 << 5) /*!< Omit advertiser's address from all Protocol Data Units ("anonymous advertising"). */
#define ESP_BLE_GAP_SET_EXT_ADV_PROP_INCLUDE_TX_PWR                       (1 << 6) /*!< Include TxPower in the extended header of the advertising Protocol Data Unit. */
#define ESP_BLE_GAP_SET_EXT_ADV_PROP_MASK                                 (0x7F)   /*!< Reserved for future use. */


/**
* @brief If extended advertising Protocol Data Unit types are being used (bit 4 = 0) then:
...
#define ESP_BLE_GAP_PHY_1M                             1 /*!< Secondary Advertisement Physical Layer is  Low Energy 1 Megabit per second. */
#define ESP_BLE_GAP_PHY_2M                             2 /*!< Secondary Advertisement Physical Layer is  Low Energy 2 Megabit per second. */
#define ESP_BLE_GAP_PHY_CODED                          3 /*!< Secondary Advertisement Physical Layer is low energy coded Physical Layer. */
...
/**
* @brief Primary Physical Layer only support 1M and low energy coded Physical Layer.
*/
#define ESP_BLE_GAP_PRI_PHY_1M     ESP_BLE_GAP_PHY_1M     /*!< Primary Physical Layer is Low Energy 1 Megabit per second. */
#define ESP_BLE_GAP_PRI_PHY_CODED  ESP_BLE_GAP_PHY_CODED  /*!< Primary Physical Layer is low energy coded Physical Layer. */
typedef uint8_t esp_ble_gap_pri_phy_t; // primary Physical Layer

#define ESP_BLE_GAP_PHY_1M_PREF_MASK                   (1 << 0) /*!< The Host prefers use the Low Energy 1 Megabit per second transmitter or receiver Physical Layer. */
#define ESP_BLE_GAP_PHY_2M_PREF_MASK                   (1 << 1) /*!< The Host prefers use the Low Energy 2 Megabit per second transmitter or receiver Physical Layer. */
#define ESP_BLE_GAP_PHY_CODED_PREF_MASK                (1 << 2) /*!< The Host prefers use the Low Energy coded Physical Layer transmitter or receiver Physical Layer. */
typedef uint8_t esp_ble_gap_phy_mask_t;

#define ESP_BLE_GAP_PHY_OPTIONS_NO_PREF                  0 /*!< The Host has no preferred coding when transmitting on the low energy Coded Physical Layer. */
#define ESP_BLE_GAP_PHY_OPTIONS_PREF_S2_CODING           1 /*!< The Host prefers that S=2 coding be used when transmitting on the low energy Coded Physical Layer. */
#define ESP_BLE_GAP_PHY_OPTIONS_PREF_S8_CODING           2 /*!< The Host prefers that S=8 coding be used when transmitting on the Low Energy Coded Physical Layer. */
typedef uint16_t esp_ble_gap_prefer_phy_options_t;

#define ESP_BLE_GAP_EXT_SCAN_CFG_UNCODE_MASK           0x01 /*!< Scan Advertisements on the Low Energy 1 Megabit per second Physical Layer. */
#define ESP_BLE_GAP_EXT_SCAN_CFG_CODE_MASK             0x02 /*!< Scan advertisements on the Low Energy coded Physical Layer. */
...
/**
* @brief Advertising Synchronous Connection policy
*/
#define ESP_BLE_GAP_SYNC_POLICY_BY_ADV_INFO       0 /*!< Synchronous Connection policy by advertising information. */
...
/**
* @brief Bluetooth 5.0, Vol 2, Part E, 7.7.65.13
*/
...
#define ESP_BLE_LEGACY_ADV_TYPE_NONCON_IND                (0x10) /*!< Advertising report with legacy none connectable indication type. */
...
#define EXT_ADV_TX_PWR_NO_PREFERENCE                      (127) /*!< Host has no preference for tx power. Extend advertising tx power, range: [-127, +126] dBm. */

/**
* @brief extended advertising parameters
*/
typedef struct {
    esp_ble_ext_adv_type_mask_t type;   /*!< extended advertising type */
    uint32_t interval_min;              /*!< extended advertising minimum interval */
    uint32_t interval_max;              /*!< extended advertising maximum interval */
    esp_ble_adv_channel_t channel_map;  /*!< extended advertising channel map */
    esp_ble_addr_type_t own_addr_type;  /*!< extended advertising own address type */
    esp_ble_addr_type_t peer_addr_type; /*!< extended advertising peer address type */
    esp_bd_addr_t peer_addr;            /*!< extended advertising peer address */
    esp_ble_adv_filter_t filter_policy; /*!< extended advertising filter policy */
    int8_t tx_power;                    /*!< extended advertising tx power */
    esp_ble_gap_pri_phy_t primary_phy;  /*!< extended advertising primary Physical Layer */
    uint8_t max_skip;                   /*!< extended advertising maximum skip */
    esp_ble_gap_phy_t secondary_phy;    /*!< extended advertising secondary Physical Layer */
    uint8_t sid;                        /*!< extended advertising sid */
    bool scan_req_notif;                /*!< extended advertising scan request event notify */
} esp_ble_gap_ext_adv_params_t;

/**
* @brief extended scan config
*/
typedef struct {
    esp_ble_scan_type_t scan_type; /*!< extended scan type */
    uint16_t scan_interval;        /*!< extended scan interval */
    uint16_t scan_window;          /*!< extended scan window */
...
*/
typedef struct {
    esp_ble_addr_type_t own_addr_type;        /*!< extended scan own address type */
    esp_ble_scan_filter_t filter_policy;      /*!< extended scan filter policy */
    esp_ble_scan_duplicate_t  scan_duplicate; /*!< extended scan duplicate scan */
    esp_ble_ext_scan_cfg_mask_t cfg_mask;     /*!< extended scan config mask */
    esp_ble_ext_scan_cfg_t uncoded_cfg;       /*!< extended scan uncoded config parameters */
    esp_ble_ext_scan_cfg_t coded_cfg;         /*!< extended scan coded config parameters */
...
typedef struct {
    ...
    uint16_t latency;             /*!< extended scan type */
...
/**
* @brief extend advertising enable parameters
*/
typedef struct {
...
/**
* @brief periodic advertising Synchronous parameters
*/
typedef struct {
    esp_ble_gap_sync_t filter_policy;       /*!< Configures the filter policy for periodic advertising Synchronous:
    ...
    #endif
    uint8_t sid;                            /*!< Service Identifier of the periodic advertising. */
...
/**
* @brief extend advertising report parameters
*/
typedef struct {
    // uint8_t props;
    ...
    esp_ble_gap_pri_phy_t primary_phy;              /*!< extend advertising primary Physical Layer */
    esp_ble_gap_phy_t secondly_phy;                 /*!< extend advertising secondary Physical Layer */
    uint8_t sid;                                    /*!< extend advertising Service ID */
    uint8_t tx_power;                               /*!< extend advertising tx power, Unit is dBm */
    int8_t rssi;                                    /*!< extend advertising Received Signal Strength Indication, Unit is dBm range: -127dBm - 20dBm */
...
typedef struct {
    ...
    int8_t rssi;                                   /*!< periodic advertising Received Signal Strength Indication */
...
*/
typedef struct {
    uint8_t status;                               /*!< periodic advertising synchronization status */
    uint16_t sync_handle;                         /*!< periodic advertising train handle */
    uint8_t sid;                                  /*!< periodic advertising Signal Strength Indication */
    esp_ble_addr_type_t addr_type;                /*!< periodic advertising address type */
    esp_bd_addr_t adv_addr;                       /*!< periodic advertising address */
    esp_ble_gap_phy_t adv_phy;                    /*!< periodic advertising advertising Physical Layer type */
...
typedef struct
{
    ...
    uint8_t                     len_of_data;           /*!< length in bytes of payload data in each packet */
    esp_ble_dtm_pkt_payload_t   pkt_payload;           /*!< packet payload type. value range: 0x00-0x07 */
    esp_ble_gap_phy_t           phy;                   /*!< the Physical Layer type used by the transmitter, coded Physical Layer with S=2:0x04 */
...
{
    uint8_t             rx_channel;            /*!<  channel for test data reception, rx_channel = (Frequency -2402)/2, tx_channel range:0x00-0x27, Frequency range: 2402 MHz to 2480 MHz */
    esp_ble_gap_phy_t   phy;                   /*!<  the Physical Layer type used by the receiver, 1M phy: 0x01, 2M phy:0x02, coded phy:0x03 */
...
*/
typedef struct {
    esp_ble_gap_past_mode_t mode;       /*!< periodic advertising synchronization transfer mode */
    ...
    uint8_t cte_type;                   /*!< periodic advertising synchronization transfer Constant Tone Extension type */
...
/**
 * @brief Generic Access Profile callback parameters union.
 */
typedef union {
    /**
...
/** if Bluetooth Low Energy 4.2 Feature Support */ 
    ...
    struct ble_scan_result_evt_param {
        ...
        esp_bt_dev_type_t dev_type;                 /*!< Device type */
        esp_ble_addr_type_t ble_addr_type;          /*!< bluetooth low energy device address type */
        esp_ble_evt_type_t ble_evt_type;            /*!< bluetooth low energy scan result event type */
        int rssi;                                   /*!< Searched device's Received Signal Strength Indication */
        uint8_t  ble_adv[ESP_BLE_ADV_DATA_LEN_MAX + ESP_BLE_SCAN_RSP_DATA_LEN_MAX];     /*!< Received Extended Inquiry Response. */
...
/** if Bluetooth Low Energy 4.2 Feature Support */ 
    ...
    struct ble_adv_clear_cmpl_evt_param {
        esp_bt_status_t status;                     /*!< Indicate advertising clear operation success status. */
    } adv_clear_cmpl;                              
#endif
//@}
    ...
    struct ble_pkt_data_length_cmpl_evt_param {
        esp_bt_status_t status;                     /*!< Indicate the set packet data length operation success status. */
        esp_ble_pkt_data_length_params_t params;    /*!< packet data length value */
   ...
   struct ble_rpa_timeout_cmpl_evt_param {
       esp_bt_status_t status;                     /*!< Indicate the set Resolvable Private Address timeout operation success status */
   ...
   struct ble_add_dev_to_resolving_list_cmpl_evt_param {
    ...
    struct ble_read_rssi_cmpl_evt_param {
        esp_bt_status_t status;                     /*!< Indicate the read advertising tx power operation success status. */
        int8_t rssi;                                /*!< The Bluetooth Low Energy remote device Received Signal Strength Indication value, the range is from -127 to 20, the unit is dbm,
                                                         if the Received Signal Strength Indication cannot be read, the Received Signal Strength Indication metric shall be set to 127. */
...
//@}
    ...
    struct ble_set_channels_evt_param {
        esp_bt_status_t stat;                       /*!< bluetooth low energy set channel status */
    } ble_set_channels;
...
/** if Bluetooth Low Energy 4.2 Feature Support */
    ...
    struct ble_read_phy_cmpl_evt_param {
        esp_bt_status_t status;                   /*!< read Physical Layer complete status */
        esp_bd_addr_t bda;                        /*!< read Physical Layer address */
        esp_ble_gap_phy_t tx_phy;                 /*!< tx Physical Layer type */
        esp_ble_gap_phy_t rx_phy;                 /*!< rx Physical Layer type */
    } read_phy;  
    /**
    ...
    struct ble_set_perf_def_phy_cmpl_evt_param {
        esp_bt_status_t status;                     /*!< Indicate performance default Physical Layer set status. */
    } set_perf_def_phy;                           
    /**
    ...
    struct ble_set_perf_phy_cmpl_evt_param {
        esp_bt_status_t status;                     /*!< Indicate performance Physical Layer set status. */
    } set_perf_phy;                               
    /**
    ...
    struct ble_periodic_adv_set_params_cmpl_param {
        esp_bt_status_t status;                    /*!< Indicate periodic advertising parameters set status. */
    ...
    struct ble_period_adv_create_sync_cmpl_param {
        esp_bt_status_t status;                  /*!< Indicate periodic advertising create Synchronization status. */
    ...
    struct ble_period_adv_sync_cancel_cmpl_param {
        esp_bt_status_t status;                  /*!< Indicate periodic advertising Synchronization cancel status. */
    ...
    struct ble_period_adv_sync_terminate_cmpl_param {
        esp_bt_status_t status;                  /*!< Indicate periodic advertising Synchronization terminate status. */
    ...
    struct ble_period_adv_add_dev_cmpl_param {
        esp_bt_status_t status;                 /*!< Indicate periodic advertising device list address status. */
    } period_adv_add_dev;                     
    /**
    ...
    struct ble_adv_terminate_param {
        uint8_t status;                   /*!< Indicate advertising terminate status. */
    ...
    struct ble_periodic_adv_sync_lost_param {
        uint16_t sync_handle;                 /*!< Synchronization handle */
    ...
    struct ble_periodic_adv_sync_estab_param {
        ...
        uint8_t sid;                         /*!< periodic advertising Service Identifier */
        esp_ble_addr_type_t adv_addr_type;   /*!< periodic advertising address type */
        esp_bd_addr_t adv_addr;              /*!< periodic advertising address */
        esp_ble_gap_phy_t adv_phy;           /*!< periodic advertising Physical Layer type */
        uint16_t period_adv_interval;        /*!< periodic advertising interval */
        uint8_t adv_clk_accuracy;            /*!< periodic advertising clock accuracy */
    ...
    struct ble_phy_update_cmpl_param {
        esp_bt_status_t status;             /*!< Physical Layer update status */
...
/** if Bluetooth Low Energy Feature Periodic Advertising Synchronization Transfer Support */ 
    ...
    struct ble_periodic_adv_sync_trans_cmpl_param {
        esp_bt_status_t status;             /*!< Periodic advertising Synchronization transfer status. */
    ...
    struct ble_periodic_adv_set_info_trans_cmpl_param {
        esp_bt_status_t status;             /*!< Periodic advertising set information transfer status. */
        esp_bd_addr_t bda;                  /*!< The remote device address. */
    ...
    struct ble_set_past_params_cmpl_param {
        esp_bt_status_t status;             /*!< Set periodic advertising Synchronization transfer params status. */
        esp_bd_addr_t bda;                  /*!< The remote device address. */
    } set_past_params;                     
    /**
    ...
    struct ble_periodic_adv_sync_trans_recv_param {
        esp_bt_status_t status;             /*!< Periodic advertising Synchronization transfer received status. */
        esp_bd_addr_t bda;                  /*!< The remote device address. */
        ...
        esp_ble_gap_phy_t adv_phy;          /*!< Periodic advertising Physical Layer. */
...
//@}
    ...
    struct ble_dtm_state_update_evt_param {
        esp_bt_status_t status;                     /*!< Indicate Direct Test Mode operation success status. */
        esp_ble_dtm_update_evt_t update_evt;        /*!< Direct Test Mode state change event, 0x00: DTM TX start, 0x01: DTM RX start, 0x02:DTM end */
    ...
    struct vendor_cmd_cmpl_evt_param {
        uint16_t        opcode;                     /*!< vendor Host Controller Interface command opcode */
...
/**
 * @brief Generic Access Profile callback function type
...
/**
 * @brief           This function is called to occur Generic Access Profile event, such as scan result.
 *
...
/**
 * @brief           This function is called to get the current Generic Access Profile callback.
 *
 * @return
...
#if (BLE_42_FEATURE_SUPPORT == TRUE)
/**
 * @brief           This function is called to override the Bluetooth Test Adapter default advertising parameters.
 *
 * @param[in]       adv_data: Pointer to User defined advertising data structure. This
...
/**
 * @brief           This function is to set maximum low energy data packet size.
 *
 * @return
 *                  - ESP_OK : success
 *                  - other  : failed
 ...
 *                  When privacy_enable is set to true, the device will use a resolvable private address (RPA) instead of a static or public address.
 *
...
/**
 * @brief           Set local Generic Access Profile appearance icon.
 *
 *
 * @param[in]       icon   - External appearance value, these values are defined by the Bluetooth Special Interest Group, please refer to:
...
*
* @param[in]        add_remove: The value is true if added the bluetooth low energy device to the white list, and false remove to the white list.
...
*
* @param[in]        bd_addr: Bluetooth Device address of the peripheral.
* @param[in]        min_conn_int: minimum preferred connection interval, unit:ms
* @param[in]        max_conn_int: maximum preferred connection interval, unit:ms
* @param[in]        slave_latency: preferred slave latency
* @param[in]        supervision_tout: preferred supervision timeout, unit:ms
*
* @return
*                   - ESP_OK : success
*                   - other  : failed
...
/**
 * @brief          This function is called to get advertising data for a specific type.
 *
 * @param[in]       adv_data - pointer of advertising data which to be resolved
 * @param[in]       type   - finding advertising data type
 * @param[out]      length - return the length of advertising data not including type
 *
 * @return          pointer of advertising data
 *
 */
...
/**
 * @brief           This function is called to read the Received Signal Strength Indication of remote device.
...
#if (BLE_42_FEATURE_SUPPORT == TRUE)
/**
 * @brief           This function is called to add a device information into the duplicate scan exceptional list.
 *
 *
 * @param[in]       type: Device information type, it is defined in esp_ble_duplicate_exceptional_info_type_t
...
/**
 * @brief           This function is called to remove a device information from the duplicate scan exceptional list.
 *
 *
 * @param[in]       type: Device information type, it is defined in esp_ble_duplicate_exceptional_info_type_t
 *                  when type is MESH_BEACON_TYPE, MESH_PROV_SRV_ADV or MESH_PROXY_SRV_ADV , device_info is invalid.
...
/**
* @brief             Set a Generic Access Profile security parameter value. Overrides the default value.
*
...
*                    (CVE-2020-26555) and 'Authentication of the low energy Legacy Pairing
*                    Protocol'.
*
...
/**
* @brief             Set a Generic Access Profile parameter value. Use this function to change
*                    the default Generic Access Profile parameter values.
*
...
*                                   what kind of bluetooth low energy security level is required for
*                                   the bluetooth low energy link if the bluetooth low energy is supported.
*
* @return            - ESP_OK : success
*                       - other  : failed
*
...
/**
* @brief           This function is called to provide the Out of Band data for
*                  Security Manager Protocol in response to ESP_GAP_BLE_OOB_REQ_EVT.
*
* @param[in]       bd_addr: Bluetooth Device address of the peer device.
* @param[in]       TK: Temporary Key value, the Temporary Key value shall be a 128-bit random number.
...
/**
* @brief           This function is called to provide the Out of Band data for
*                  Security Manager Protocol in response to ESP_GAP_BLE_SC_OOB_REQ_EVT.
*
* @param[in]       bd_addr: Bluetooth Device address of the peer device.
* @param[in]       p_c: Confirmation value, it shall be a 128-bit random number.
* @param[in]       p_r: Randomised value, it should be a 128-bit random number.
*
* @return          - ESP_OK : success
*                  - other  : failed
*
...
/**
* @brief           This function is called to create the  Out of Band data for
*                  Security Manager Protocol when secure connection.
*
* @return          - ESP_OK : success
*                  - other  : failed
*
...
*                  Generic Attribute Profile - Client may have multiple virtual GATT server connections when multiple app_id registered.
...
*
*
* @param[in]       remote_device : Bluetooth Device address of the peer device.
*
* @return            - ESP_OK : success
*                    - other  : failed
*
...
/**
* @brief            bluetooth low energy set channels
*
...
/**
* @brief           This function is called to authorized a link after Authentication(Man-in-the-middle protection).
*
* @param[in]       bd_addr: Bluetooth Device address of the peer device.
...
*                  for the transmitter Physical Layer and receiver Physical Layer to be used for all subsequent connections
*                  over the low energy transport.
*
* @param[in]       tx_phy_mask : indicates the transmitter Physical Layers that the Host prefers the Controller to use
* @param[in]       rx_phy_mask : indicates the receiver Physical Layers that the Host prefers the Controller to use
*
* @return            - ESP_OK : success
*                    - other  : failed
*
...
/**
* @brief           This function is used to set the Physical Layer preferences for the connection identified by the remote address.
*                  The Controller might not be able to make the change (e.g. because the peer does not support the requested Physical Layer)
*                  or may decide that the current Physical Layer is preferable.
*
* @param[in]       bd_addr : remote address
* @param[in]       all_phys_mask : a bit field that allows the Host to specify
* @param[in]       tx_phy_mask : a bit field that indicates the transmitter Physical Layers that the Host prefers the Controller to use
* @param[in]       rx_phy_mask : a bit field that indicates the receiver Physical Layers that the Host prefers the Controller to use
* @param[in]       phy_options : a bit field that allows the Host to specify options for Physical Layers
*
* @return            - ESP_OK : success
*                    - other  : failed
*
...
/**
* @brief           This function is used to set the data used in advertising Protocol Data Units that have a data field.
*
...
/**
* @brief           This function is used to provide scan response data used in scanning response Protocol Data Units.
*
* @param[in]       instance : identifies the advertising set whose response data are being configured.
* @param[in]       length : response data length
...
* @param[in]       ext_adv : advertising parameters
*
* @return            - ESP_OK : success
*                    - other  : failed
*
...
* @param[in]       params : periodic advertising parameters
*
* @return            - ESP_OK : success
*                    - other  : failed
*
...
/**
* @brief           This function is used to set the data used in periodic advertising Protocol Data Units.
*
* @param[in]       instance : identifies the advertising set whose periodic advertising parameters are being configured.
...
*
* @param[in]       params : synchronization parameters
*
* @return            - ESP_OK : success
*                    - other  : failed
*
...
* @param[in]       sid : Advertising Service Identifier subfield in the Advertising Data Indicator field used to identify the Periodic Advertising.
*
* @return            - ESP_OK : success
*                    - other  : failed
*
...
* @param[in]       addr_type : address type
* @param[in]       addr : Device Address
* @param[in]       sid : Advertising Service Identifier subfield in the ADI field used to identify the Periodic Advertising.
*
* @return            - ESP_OK : success
*                    - other  : failed
*
...
/**
* @brief           This function is used to set Auxiliary connection parameters.
*
* @param[in]       addr : device address
* @param[in]       phy_mask : Indicates the Physical Layer(s) on which the advertising packets should be received on the primary advertising channel and the Physical Layers for which connection parameters have been specified.
* @param[in]       phy_1m_conn_params : Scan connectable advertisements on the low energy 1M Physical Layer. Connection parameters for the low energy 1M Physical Layer are provided.
* @param[in]       phy_2m_conn_params : Connection parameters for the low energy 2M Physical Layer are provided.
* @param[in]       phy_coded_conn_params : Scan connectable advertisements on the LE Coded Physical Layer. Connection parameters for the low energy Coded Physical Layer are provided.
*
* @return            - ESP_OK : success
*                    - other  : failed
*
...
*
* @param[in]       sync_handle : Handle of periodic advertising Synchronous.
...
/**
* @brief           This function is used to transfer periodic advertising Synchronous.
*
...
/**
* @brief           This function is used to set periodic advertising Synchronous transfer params.
*
* @param[in]       addr : Peer device address.
* @param[in]       params : Params of periodic advertising Synchronous transfer.
*
* @return            - ESP_OK : success
*                    - other  : failed
*
...
/**
* @brief           This function is used to start a test where the Device Under Test generates reference packets
*                  at a fixed interval.
*
* @param[in]       tx_params : Direct Test Mode Transmitter parameters.
*
* @return            - ESP_OK : success
*                    - other  : failed
*
...
/**
* @brief           This function is used to start a test where the Device Under Test receives test reference packets
*                  at a fixed interval.
*
* @param[in]       rx_params : Direct Test Mode Receiver parameters.
*
* @return            - ESP_OK : success
*                    - other  : failed
*
...
/**
 * @brief           This function is called to send vendor Host Controller Interface command.
 *
 *
 *
 * @param[in]       vendor_cmd_param: vendor Host Controller Interface command parameters
 *
 * @return
 *                  - ESP_OK : success
 *                  - other  : failed

 ...
 *        - Directed advertising can not advertising `adv_data`.
 */
typedef enum {
    ADV_TYPE_IND                = 0x00, /*!< Connectable and scannable undirected advertising (ADV_IND PDU).
                                             - Advertising will send the connectable and scannable undirected packet,
    ...
    ADV_TYPE_SCAN_IND           = 0x02, /*!< Scannable undirected advertising (ADV_SCAN_IND PDU). 
                                             - Advertising will send the scannable undirected event type,
                                             ...
                                             - Direct connection is not allowed. */
    ADV_TYPE_NONCONN_IND        = 0x03, /*!< Non connectable undirected advertising (ADV_NONCONN_IND PDU). 
                                             - Advertising will send the non-connectable and non-scannable undirected packet,
    ...
    ADV_TYPE_DIRECT_IND_LOW     = 0x04  /*!< Connectable low duty cycle directed advertising (ADV_DIRECT_IND PDU, low duty cycle).
                                             - Advertising will send the connectable directed packet,
 ...
 * These security actions determine the level of encryption and authentication required for BLE connections.
 */
typedef enum {
    ESP_BLE_SEC_ENCRYPT = 1, /*!< Relates to BTA_DM_BLE_SEC_ENCRYPT in bta/bta_api.h (Reserved for internal use).
                                ...
                                If the device hasn't bonded, the stack will use the default authentication request (same as no Man-In-The-Middle (MITM) authentication request) 
                                set by the user using the esp_ble_gap_set_security_param function. */

    ESP_BLE_SEC_ENCRYPT_NO_MITM, /*!< Relates to BTA_DM_BLE_SEC_ENCRYPT_NO_MITM in bta/bta_api.h (Reserved for internal use).
                                    If the device has already bonded, the stack will check if the Long Term Key (LTK) meets the authentication request, and if so, use it to encrypt communication with the remote device directly. 
                                    Otherwise, it will re-pair with the remote device.
                                    If the device hasn't bonded, the stack will use no MITM authentication request in the current link instead of the MITM authentication set by the user in the esp_ble_gap_set_security_param function. */

    ESP_BLE_SEC_ENCRYPT_MITM, /*!< Relates to BTA_DM_BLE_SEC_ENCRYPT_MITM in bta/bta_api.h (Reserved for internal use).
                                If the device has already bonded, the stack will check if the Long Term Key (LTK) meets the authentication request, and if so, use it to encrypt communication with the remote device directly. 
                                Otherwise, it will re-pair with the remote device.
                                If the device hasn't bonded, the stack will use MITM authentication request in the current link instead of the MITM authentication set by the user in the esp_ble_gap_set_security_param function. */
} esp_ble_sec_act_t;

/**
 * @brief These annotations provide clearer explanations of each enum member's role in BLE security management.
 */
typedef enum {
    ESP_BLE_SM_PASSKEY = 0,             /*!< Authentication method using passkey input on the local device. */
    ESP_BLE_SM_AUTHEN_REQ_MODE,         /*!< Authentication mode requirements of the local device. */
    ESP_BLE_SM_IOCAP_MODE,              /*!< Input/output capabilities of the local device in pairing. */
    ESP_BLE_SM_SET_INIT_KEY,            /*!< Initiator key distribution/generation during pairing. */
    ESP_BLE_SM_SET_RSP_KEY,             /*!< Responder key distribution/generation during pairing. */
    ESP_BLE_SM_MAX_KEY_SIZE,            /*!< Maximum encryption key size supported by the device. */
    ESP_BLE_SM_MIN_KEY_SIZE,            /*!< Minimum encryption key size required from the peer device. */
    ...
    ESP_BLE_SM_ONLY_ACCEPT_SPECIFIED_SEC_AUTH,           /*!< Accept only specified security authentication requirements. */
    ESP_BLE_SM_OOB_SUPPORT,             /*!< Out of Band (OOB) data support for pairing. */
    ESP_BLE_APP_ENC_KEY_SIZE,           /*!< Size of the application encryption key used. */
...
typedef struct {
    ...
    uint16_t adv_int_max;                   /*!< Maximum advertising interval for undirected and low duty cycle directed advertising.
                                                 ...
                                                 - ADV_Int_MAX shall be equal to or greater than ADV_Int_MIN, when both are equal, the interval is definite,
                                                   otherwise it is any value in the interval.
                                                 - Note: There will be a random delay in each interval range 0 to 10ms.*/
 ...
 *          the remaining configuration items are not selected indicate that the current package does not contain the configuration item.
 *        - E.g.: Send an empty advertisement package, don't have to set anything up.
 *               Send an empty scan response package, set `set_scan_rsp` to `true`.
 */
typedef struct {
    ...
    bool include_name;              /*!< Whether advertising data includes the device name or not.
                                         - True indicates the device name is included, false indicates not included.
                                         - Please use `esp_ble_gap_set_device_name` to set device name. */
    bool include_txpower;           /*!< Whether advertising data includes TX power or not.
                                         - True indicates TX power is included, false indicates not included.
                                         - Please use `esp_ble_tx_power_set` to set TX power. */
    int min_interval;               /*!< Slave preferred connection minimum interval.
                                         - Unit: 1.25ms.
                                         - Range: 0x0006 to 0x0C80 (7.5ms to4 s). */
    ...
    uint16_t service_uuid_len;      /*!< Service UUID length in bytes. */
    uint8_t *p_service_uuid;        /*!< Pointer to the service UUID array.
                                         ...
                                         - The UUID in the list is of 16/32/128-bit Service UUIDs. */
 ...
 */
typedef enum {
    BLE_SCAN_TYPE_PASSIVE   =   0x0, /*!< Passive scan. No scanning Protocol Data Unit (PDU) shall be sent. */
    BLE_SCAN_TYPE_ACTIVE    =   0x1, /*!< Active scan. Scanning PDU may be sent. */
} esp_ble_scan_type_t;
...
typedef enum {
    BLE_SCAN_FILTER_ALLOW_ALL           = 0x0,  /*!< Accept all:
                                                  1. Advertising packets except directed advertising packets not addressed to this device (default).
                                                   */
    BLE_SCAN_FILTER_ALLOW_ONLY_WLST     = 0x1,  /*!< Accept only:
                                                  1. Advertising packets from devices where the advertiser's address is in the White list.

                                                  2. Directed advertising packets not addressed to this device shall be ignored.
                                                   */
    BLE_SCAN_FILTER_ALLOW_UND_RPA_DIR   = 0x2,  /*!< Accept all:
                                                  1. Undirected advertisement packets.

                                                  2. Directed advertising packets where the initiator address is a resolvable private address.
...
/* if Bluetooth Low Energy 5.0 Feature Support */  
    ...
    BLE_SCAN_DUPLICATE_ENABLE_RESET,             /*!< Duplicate filtering enabled, reset for each scan period, only supported in bluetooth low energy 5.0. */
 ...
 */
typedef struct {
    esp_ble_scan_type_t      scan_type;              /*!< Scan type, specifies whether to use passive (default) or active (0x1) scanning. */
    esp_ble_addr_type_t      own_addr_type;          /*!< Owner address type, specifies the type of Bluetooth address used by the scanning device. */
    esp_ble_scan_filter_t    scan_filter_policy;     /*!< Scan filter policy, defines the criteria for accepting advertisement packets. */
    uint16_t                 scan_interval;          /*!< Scan interval, the time between the start of consecutive scans.
                                                          - Unit: 0.625ms.
                                                          - Range: `0x0004` to `0x4000` (2.5ms to 10.24s).
                                                          - Default: 0x0010 (10ms). */
    uint16_t                 scan_window;            /*!< Scan window, the duration of each scan period.
                                                          - Scan_Window must be less than or equal to Scan_Interval!
                                                          - Unit: 0.625ms.
                                                          - Range: `0x0004` to `0x4000` (2.5ms to 10.24s).
                                                          - Default: 0x0010 (10ms). */
...
/**
 * @brief Connection parameters information for a Bluetooth Low Energy (BLE) connection.
 */
typedef struct {
    uint16_t interval;                  /*!< Connection interval, defines how frequently the devices will exchange data.
                                           Time = N * 1.25ms
...
/**
 * @brief Parameters for updating BLE connection parameters.
 */
typedef struct {
    esp_bd_addr_t bda;                              /*!< Bluetooth device address for which the connection parameters are being updated. */
    uint16_t min_int;                               /*!< Minimum connection interval, specifies the shortest time between connection events.
                                                       Time = N * 1.25ms
                                                       Range: 0x0006 to 0x0C80
                                                       Time Range: 7.5ms to 4s. */
    uint16_t max_int;                               /*!< Maximum connection interval, specifies the longest time between connection events.
                                                       Time = N * 1.25ms
                                                       Range: 0x0006 to 0x0C80
                                                       Time Range: 7.5ms to 4s. */
    uint16_t latency;                               /*!< Slave latency for the connection, specifies the number of connection events the slave can skip.
...
/**
 * @brief Bluetooth Low Energy (BLE) encryption keys.
 * The key type used for encryption in BLE security.
 */
typedef struct {
    ...
    uint8_t              sec_level;    /*!< Security level of the link.
                                           ...
                                           */
    uint8_t              key_size;     /*!< Key size used in the security link.
                                           Value range: 7 to 16 bytes.
                                           Specifies the length of the encryption key in bytes. */
} esp_ble_penc_keys_t;

/**
 * @brief Bluetooth Low Energy (BLE) connection signature resolving keys.
...
/**
 * @brief Bluetooth Low Energy (BLE) pairing identification keys.
...
typedef struct {
    esp_ble_key_mask_t    key_mask;       /*!< The key mask to indicate which keys are present.
                                                Helps in determining which keys are available for a specific bonded device. */
    esp_ble_penc_keys_t   penc_key;       /*!< Received peer encryption key (Long Term Key, LTK).
...
typedef struct {
    ...
    esp_ble_bond_key_info_t bond_key;     /*!< Bond key information associated with the bonded device.
                                                Includes encryption and resolving keys. */
} esp_ble_bond_dev_t;          
...
typedef struct {
    ...
    esp_bt_octet16_t       dhk;                 /*!< The 16 bytes of the Diffie-Hellman key (DHK).
                                                   Used for secure key exchange in BLE pairing. */
...
/**
 * @brief Enumeration for different types of exceptional scan information in Bluetooth Low Energy (BLE).
 * This enumeration defines various types of exceptional scan information that can be used to manage
 * specific types of BLE advertising and scanning data. These types help in identifying and categorizing
...
#define ESP_BLE_LEGACY_ADV_TYPE_NONCON_IND                (0x10)    /*!< Legacy Non-Connectable Advertising Indication Report
                                                                        ...
                                                                        Non-connectable advertisements are used for broadcasting information to nearby devices without establishing a connection. */
 ...
 * 
 *  - Please use `esp_err_t esp_ble_gap_config_adv_data` to set advertising data or scan response data.
 *  - Please use `esp_ble_gap_config_scan_rsp_data_raw` to set scan response raw data.
 *
 ...
 *                  - [Data Type N]: Type of the advertising data segment N, as defined by the BLE specification.
 ...
 *
 * @return          
 *                  - ESP_OK : Success, raw advertising data configured successfully.
 *                  - ESP_FAIL : Failed, raw advertising data could not be configured. Refer to error codes for details.
 * 
 ...
 * 
 *  - Please use `esp_err_t esp_ble_gap_config_adv_data` to set advertising data or scan response data.
 *  - Please use `esp_ble_gap_config_adv_data_raw` to set scan response raw data.
 *
 ...
 *                  - [Data Type N]: Type of the scan response data segment N, as defined by the BLE specification.
 *                  - [Data N]: Actual data for segment N.
 * @param[in]       raw_data_len: Length of the raw scan response data in bytes. The value must be less than 31 bytes.
 *
 * @return          
 *                  - ESP_OK : Success, raw scan response data configured successfully.
 *                  - ESP_FAIL : Failed, raw scan response data could not be configured. Refer to error codes for details.
 * 
 ...
 *
 * @note
 *                  - This api can also be used in changing advertising parameters.
 ...
 *
 *  - Configure the parameters that determine how the LE device scans for nearby LE devices.
 * 
 ...
 *
 * @note
 *                  - The `scan_params` structure should not be freed until the callback for `set_scan_params` is received.
 *                  - The function triggers `ESP_GAP_BLE_SCAN_PARAM_SET_COMPLETE_EVT`.
 */
esp_err_t esp_ble_gap_set_scan_params(esp_ble_scan_params_t *scan_params);

/**
 * @brief  Start a scanning procedure to search for Bluetooth Low Energy (LE) devices advertising in the air.
 *
...
/**
 * @brief  Stop a scanning procedure to search for Bluetooth Low Energy (LE) devices advertising in the air.
 *
 *  - It halts the ongoing scanning procedure that was initiated by calling `esp_ble_gap_start_scanning`.
 *
 * @return 
 *                  - ESP_OK : Success.
 *                  - ESP_FAIL : Failed.
...
/**
 * @brief  Get advertising data for a specific type.
 *
 *  - Advertising data in Bluetooth Low Energy (LE) can contain various types of information such as 
 *    device name, appearance, manufacturer data, etc. This function helps in extracting the required 
 *    type of advertising data for further processing or analysis.
 *                  
 * @param[in]       adv_data: Pointer to the buffer containing the complete advertising data.
 * @param[in]       type: The type of advertising data to find.
 ...
 *
 * @note            If the function returns `NULL`, it means the specified type of advertising data was not found in the
 *                  provided buffer.
 */
 ...
 *                  This function allows the user to add a specific device's information to a list of exceptions 
 *                  for duplicate scans. Devices in this list will be treated as exceptions, and their duplicate 
 *                  scan results will be handled according to the specified type.
 *                  Note that for certain types (e.g., MESH_BEACON_TYPE, MESH_PROV_SRV_ADV, or MESH_PROXY_SRV_ADV), 
 *                  the `device_info` parameter is not used and should be considered invalid.
 *
 * @param[in]       type: The type of device information to add. This is defined in the `esp_ble_duplicate_exceptional_info_type_t` enumeration.
 ...
 *                  This parameter is used to specify the device details except for the aforementioned types where it is invalid.
 *
 * @return          
 *                  - ESP_OK : Success, the device information was added to the duplicate scan exceptional list successfully.
 *                  - ESP_FAIL : Failed, the device information could not be added to the list. Refer to error codes for details.
 * 
 * @note            The function triggers the following event:
 *                  - `ESP_GAP_BLE_UPDATE_DUPLICATE_EXCEPTIONAL_LIST_COMPLETE_EVT`, Subevent code£º-`ESP_BLE_DUPLICATE_EXCEPTIONAL_LIST_ADD`
 */
 ...
 *                  This function allows the user to remove a specific device's information from the list of exceptions 
 *                  for duplicate scans. Devices in this list are treated as exceptions during scanning, and this function 
 *                  will remove the specified device's information from that list.
 *                  Note that for certain types (e.g., MESH_BEACON_TYPE, MESH_PROV_SRV_ADV, or MESH_PROXY_SRV_ADV), 
 *                  the `device_info` parameter is not used and should be considered invalid.
 *
 * @param[in]       type: The type of device information to remove. This is defined in the `esp_ble_duplicate_exceptional_info_type_t` enumeration.
 ...
 *                  This parameter specifies the device details to be removed from the list except for the types where it is invalid.
 *
  * @return          
 *                  - ESP_OK : Success, the device information was removed from the duplicate scan exceptional list successfully.
 *                  - ESP_FAIL : Failed, the device information could not be removed from the list. Refer to error codes for details.
 * 
 * @note            The function triggers the following event:
 *                  - `ESP_GAP_BLE_UPDATE_DUPLICATE_EXCEPTIONAL_LIST_COMPLETE_EVT`, Subevent code£º-`ESP_BLE_DUPLICATE_EXCEPTIONAL_LIST_ADD`
 */
 ...
 *                  The exceptional list contains device information that is treated as exceptions during duplicate scanning.
 *                  By calling this function, you clear all entries from this list.
 *
 * @param[in]       list_type: The type of duplicate scan exceptional list to clean. This parameter is defined in the
 *                  `esp_duplicate_scan_exceptional_list_type_t` enumeration and can include one or more types:
 ...
 *
  * @return          
 *                  - ESP_OK : Success, the duplicate scan exceptional list has been cleaned.
 *                  - ESP_FAIL : Failed, the exceptional list could not be cleaned. Refer to error codes for details.
 * 
 * @note            The function triggers the following event:
 *                  - `ESP_GAP_BLE_UPDATE_DUPLICATE_EXCEPTIONAL_LIST_COMPLETE_EVT`, Subevent code£º-`ESP_BLE_DUPLICATE_EXCEPTIONAL_LIST_CLEAN`
 */
esp_err_t esp_ble_gap_clean_duplicate_scan_exceptional_list(esp_duplicate_scan_exceptional_list_type_t list_type);
#endif// #if (BLE_42_FEATURE_SUPPORT == TRUE)

/**
 * @brief  Update the Bluetooth Low Energy (BLE) connection parameters for an existing connection.
 *
 ...
 *
 * @note
 *                  - This function can only be used when a LE connection is already established.
 *                  - The function triggers `ESP_GAP_BLE_UPDATE_CONN_PARAMS_EVT`.
 */
esp_err_t esp_ble_gap_update_conn_params(esp_ble_conn_update_params_t *params);

/**
 * @brief  Set the maximum data packet size.
 *
 *  - Learning more in spec: BLUETOOTH CORE SPECIFICATION Version 5.4 Vol 6, Part B.
 *
 * @param[in]       remote_device: Address of the remote LE device for which the data packet size is being set.
 *
 ...
 *
 * @note
 *                  - Ensure that both devices agree on the packet size. The Controller may use smaller or larger values based on local information.
...
/**
 * @brief           This function adds a device to the resolving list used to generate and resolve Resolvable Private Addresses (RPAs)
 *                  in the Bluetooth Controller. The resolving list is utilized to manage devices with which the Controller has
 *                  previously exchanged Identity Resolving Keys (IRKs). The added device will be used to generate and resolve
 *                  Resolvable Private Addresses during Bluetooth communication.
 *
 ...
 *
 * @return          
 *                  - ESP_OK : Success, the device was added to the resolving list successfully.
 *                  - ESP_FAIL : Failed, the device could not be added to the resolving list. Refer to the error codes for details.
 *
...
/**
 * @brief           This function clears the random address that was previously set for the application.
 *                  It removes the current random address configuration, which may have been used for
 *                  privacy or other purposes. After calling this function, the Bluetooth device will
 *                  no longer use the previously set random address.
 *                  
 *                  The function returns the result of the operation. If the address clearing was successful,
 *                  the device will revert to using the default address or other address configuration.
 *
 * @return          
 *                  - ESP_OK : Success, the random address was cleared successfully.
 *                  - ESP_FAIL : Failed, the random address could not be cleared. Refer to the error codes for details.
 *                  
 * @note            Clearing the random address will not affect other Bluetooth settings or connections.
 *                  It is important to configure a new address if needed before starting advertising or scanning
 *                  operations again.
 */
esp_err_t esp_ble_gap_clear_rand_addr(void);

/**
 * @brief           This function enables or disables privacy features, including address resolution, on the local Bluetooth device.
 *                  When privacy is enabled (`privacy_enable` set to `true`), the device will use a Resolvable Private Address (RPA)
 *                  rather than a static or public address. This helps to enhance privacy by periodically changing the address 
 *                  while still allowing for address resolution.
 *                  
 *                  When privacy is disabled (`privacy_enable` set to `false`), the device will use its static or public address
 *                  as usual, without employing RPA. This can be useful in scenarios where privacy is not a concern or when
 *                  privacy-related operations are not required.                 
 *
 ...
 *
 * @return          
 *                  - ESP_OK : Success, privacy configuration applied successfully.
 *                  - ESP_FAIL : Failed, privacy configuration could not be applied. Refer to the error codes for details.
 *                  
 * @note            Enabling privacy can impact the device's discoverability and connection process, as other devices 
 *                  may need to resolve the RPA to establish a connection. Ensure that privacy settings align with your application's
 *                  requirements.
 * 
 *                  The function triggers the following event:
 *                  - `ESP_GAP_BLE_SET_LOCAL_PRIVACY_COMPLETE_EVT`: Triggered when the local device privacy settings are complete. 
 *                  This event is used to notify the application that the local device privacy settings are complete.
 */
esp_err_t esp_ble_gap_config_local_privacy(bool privacy_enable);

/**
 * @brief           This function sets the local Generic Access Profile (GAP) appearance icon.
 *                  The appearance icon is used to provide a human-readable indication of the device's type or appearance 
 *                  in Bluetooth profiles and applications. This helps other devices or applications to identify or categorize
 *                  the device based on its appearance. The values for the appearance icon are defined by the Bluetooth Special
 *                  Interest Group (SIG).
 *
 ...
 *
 * @return          
 *                  - ESP_OK : Success, the local appearance icon was set successfully.
 *                  - ESP_FAIL : Failed, the local appearance icon could not be set. Refer to error codes for details.
 *                  
 * @note            Ensure that the appearance value you set is valid and corresponds to a predefined value from the 
 *                  Bluetooth SIG specifications to ensure proper functionality and compatibility with other Bluetooth devices.
 */
esp_err_t esp_ble_gap_config_local_icon(uint16_t icon);

/**
 * @brief           Add or remove a device from the whitelist.
 *                  The whitelist is a list of Bluetooth Low Energy (BLE) devices that are allowed or disallowed for
 *                  connection or interaction. This function allows you to manage the whitelist by adding or removing
 *                  devices based on their address.
 *                  
 *                  When `add_remove` is set to true, the specified device is added to the whitelist. When it is set to
 *                  false, the specified device is removed from the whitelist. The `remote_bda` parameter specifies the
 *                  Bluetooth device address (BD_ADDR) of the device to be added or removed, and `wl_addr_type` specifies
 ...
 * @param[in]       wl_addr_type: The address type of the device being added or removed. This can be either
 *                  `BLE_ADDR_TYPE_PUBLIC` for a public address or `BLE_ADDR_TYPE_RANDOM` for a random address.
 *
 * @return          
 *                  - ESP_OK : Success, the device was added to or removed from the whitelist successfully.
 *                  - ESP_FAIL : Failed, the device could not be added to or removed from the whitelist. Refer to error codes
 *                    for details.
 * 
 * @note            Managing the whitelist helps control which devices can connect to or interact with your BLE
 *                  device. Ensure that you correctly specify the address type and address to avoid accidental removal or
 *                  addition of unintended devices.
 *
 *                  The function triggers the following event:
 *                  - `ESP_GAP_BLE_UPDATE_WHITELIST_COMPLETE_EVT`: This event indicates the result of the whitelist update
 *                    operation. It provides information about whether the device was successfully added or removed from the
 *                    whitelist.
 */
 ...
 *                  The whitelist is a list of Bluetooth Low Energy (BLE) devices that are allowed or disallowed for
 *                  connection or interaction. This function removes all devices from the whitelist, effectively
 *                  clearing the list of previously allowed or disallowed devices.
 *                  
 *                  After calling this function, the whitelist will be empty, and no devices will be on the list.
 *                  This operation may be useful for resetting the device's connection policy or starting fresh
 *                  with a new whitelist configuration.
 *
 * @return          
 *                  - ESP_OK : Success, the whitelist was cleared successfully.
 *                  - ESP_FAIL : Failed, the whitelist could not be cleared. Refer to error codes for details.
 *
 ...
 *                  This function retrieves the current size of the whitelist, which indicates the number of entries
 *                  that the whitelist can hold. The whitelist size defines how many devices can be added to or removed
 *                  from the whitelist. This information is useful for understanding the capacity of the whitelist
 *                  and managing device entries accordingly.
 *                  
 ...
 *
 * @return          
 *                  - ESP_OK : Success, the whitelist size was successfully retrieved.
 *                  - ESP_FAIL : Failed, the whitelist size could not be retrieved. Refer to error codes for details.
 */
...
/**
 * @brief           This function is used to set the preferred connection parameters for a Bluetooth Low Energy (BLE) connection.
 *                  This configuration is applied before initiating a connection to a peripheral device. It is applicable only 
 *                  when the device is in the master role. 
 *                  
 *                  This function allows you to specify these parameters to optimize the connection based on your application's 
 *                  needs and to ensure compatibility with the peripheral device.
 *                  
 *                  After calling this function, the preferred connection parameters will be used when a connection is established 
 *                  with the specified peripheral device.
 *
 * @param[in]       bd_addr: The Bluetooth Device Address (BD_ADDR) of the peripheral device for which the preferred connection 
 *                  parameters are being set.
 * @param[in]       min_conn_int: Minimum preferred connection interval, in ms. This defines the shortest time between
 *                  two consecutive connection events.
 * @param[in]       max_conn_int: Maximum preferred connection interval, in ms. This defines the longest time between
 *                  two consecutive connection events.
 * @param[in]       slave_latency: Preferred slave latency. This is the number of connection events that the slave device is allowed
 *                  to skip.
 * @param[in]       supervision_tout: Preferred supervision timeout, in ms. This defines the time period within which
 *                  the connection must be maintained to avoid disconnection.
 *
 * @return          
 *                  - ESP_OK : Success, the preferred connection parameters were set successfully.
 *                  - ESP_FAIL : Failed, the preferred connection parameters could not be set. Refer to error codes for details.
 *                  
 * @note            This function does not trigger an event directly. The connection parameters are applied when the connection
 *                  is initiated. You can monitor the connection status and parameters through other events related to BLE connections.
 */
 ...
 *                  This function allows you to set or update the device name that will be used for identification 
 *                  purposes. The device name is a human-readable string that can be displayed by other devices 
 *                  to identify your BLE device.
 *
 * @param[in]       name: The device name to be set. This should be a null-terminated string for no more than 31 bytes.
 *
 * @return          
 *                  - ESP_OK : Success, the device name was set successfully.
 *                  - ESP_FAIL : Failed, the device name could not be set. Refer to error codes for details.
 *                  
...
/**
 * @brief           Retrieve the device name of the local Bluetooth Low Energy (BLE) device.
 *                  This function is used to get the current device name that has been assigned to the local BLE device.
 *                  The device name is a human-readable string that identifies your BLE device and can be displayed
 *                  by other BLE devices or applications to recognize it. 
 *
 * @return          
 *                  - ESP_OK : Success, the device name was retrieved successfully.
 *                  - ESP_FAIL : Failed, the device name could not be retrieved. Refer to the error codes for details.
 *                  
...
/**
 * @brief           Retrieve the local Bluetooth Low Energy (BLE) device's currently used address and its type.
 *                  This function provides the current address and address type that the local BLE device is using. 
 *                  The address can be either a public address or a random address, depending on the device's configuration.
 *                  
 ...
 *
 * @param[out]      local_used_addr: Pointer to a buffer where the current local BLE address will be stored. This buffer
 ...
 *
 * @return          
 *                  - ESP_OK : Success, the local used address and address type were retrieved successfully.
 *                  - ESP_FAIL : Failed, the local used address and address type could not be retrieved. Refer to the error
 *                    codes for details.
 * 
 * @note            The function triggers the following event:
 *                  - `ESP_GAP_BLE_GET_DEV_NAME_COMPLETE_EVT`: Triggered when getting the Bluetooth device name is complete, notifies the application of the device name result.
 */
esp_err_t esp_ble_gap_get_local_used_addr(esp_bd_addr_t local_used_addr, uint8_t * addr_type);

/**
 * @brief           Request to read the Received Signal Strength Indication (RSSI) of a remote Bluetooth device.
 *                  The Received Signal Strength Indication is a measurement of the signal strength of the Bluetooth 
 *                  signal received from a remote device. This function triggers a request to read the RSSI value for
 *                  the specified remote device address. Once the RSSI reading is complete, the result will be returned
 *                  in the GAP (Generic Access Profile) callback function with the `ESP_GAP_BLE_READ_RSSI_COMPLETE_EVT` event.
 *
 * @param[in]       remote_addr: The Bluetooth Device Address (BD_ADDR) of the remote device whose RSSI value is to be read.
 *                  This address uniquely identifies the remote device in the Bluetooth network.
 *
 * @return          
 *                  - ESP_OK : Success, the RSSI reading request was initiated successfully.
 *                  - ESP_FAIL : Failed, the RSSI reading request could not be initiated. Refer to error codes for details.
 *
 * @note            The RSSI value can provide insight into the quality of the connection with the remote device.
 *                  The function triggers the following event:
 *                  - `ESP_GAP_BLE_READ_RSSI_COMPLETE_EVT`: This event indicates that the RSSI reading request has completed.
 *                    The event provides information about the RSSI value and the result of the operation.
 */
 ...
 *                  Secure connection is highly recommended to avoid major vulnerabilities such as 
 *                  'Impersonation in the Pin Pairing Protocol' (CVE-2020-26555) and 'Authentication of the Low Energy Legacy Pairing Protocol'.
 *                  To accept only secure connection mode, follow these steps:
 *
 *                  1. Set the bit `ESP_LE_AUTH_REQ_SC_ONLY` (param_type is `ESP_BLE_SM_AUTHEN_REQ_MODE`), 
 ...
 *
 * @param[in]       param_type : The type of the parameter to be set. This is defined in the `esp_ble_sm_param_t` enumeration.
 * @param[in]       value  : Pointer to the parameter value to be set.
 * @param[in]       len : The length of the parameter value.
 *
  * @return          
 *                  - ESP_OK : Success, the security parameter was set successfully.
 *                  - ESP_FAIL : Failed, the security parameter could not be set. Refer to error codes for details.
 */
esp_err_t esp_ble_gap_set_security_param(esp_ble_sm_param_t param_type, void *value, uint8_t len);


/**
 * @brief           Grant or deny security request access from a peer Bluetooth device.
 *
 *                  This function is used to respond to a security request from a peer device.
 *                  The security request typically involves pairing or encryption setup.
 *
 * @param[in]       bd_addr : Bluetooth Device address of the peer. This is a 6-byte address that uniquely identifies the Bluetooth device.
 * @param[in]       accept  : Set to `true` if thesecurity request, otherwise `false` if deny the security request.
 *
  * @return          
 *                  - ESP_OK : Success, the security request was responded to successfully.
 *                  - ESP_FAIL : Failed, the security request could not be responded to. Refer to error codes for details.
 *
 * @note            This function related to the following event:
 *                  - `ESP_GAP_BLE_AUTH_CMPL_EVT`: Indicates that the authentication process has completed.
 *                  - `ESP_GAP_BLE_SEC_REQ_EVT`: Triggered when the BLE device sends a security request, notifying the application to process the security request.
 */
...
/**
 * @brief             Set a Generic Access Profile (GAP) security parameter value to enable encryption for a peer device.
 *                    Use this function to change the default GAP security parameter values and initiate encryption.
 *
 * @param[in]         bd_addr : The address of the peer Bluetooth device that needs encryption. This is a 6-byte address 
 *                              that uniquely identifies the Bluetooth device.
 * @param[in]         sec_act : The security action indicating the required Bluetooth Low Energy (BLE) security level for 
 *                              the BLE link. The available security actions are defined in `esp_ble_sec_act_t` and can include
 *                              levels such as Just Works, Passkey Entry, Numeric Comparison, and Out of Band (OOB).
 *
  * @return          
 *                    - ESP_OK : Success, encryption was set successfully.
 *                    - ESP_FAIL : Failed, encryption could not be set. Refer to the error codes for details.
 */
esp_err_t esp_ble_set_encryption(esp_bd_addr_t bd_addr, esp_ble_sec_act_t sec_act);

/**
 * @brief          Reply with the passkey value to the peer device during the legacy pairing process.
 *                 This function is used to respond to a passkey entry request from the peer device.
 *
 * @param[in]      bd_addr : The Bluetooth Device address of the peer. This is a 6-byte address 
 *                           that uniquely identifies the Bluetooth device.
 * @param[in]      accept  : Indicates whether the passkey entry was successful or declined. 
 *                           Set to `true` if the passkey entry was successful, otherwise `false` if declined.
 * @param[in]      passkey : The passkey value, which must be a 6-digit number (e.g., 000000 to 999999).
 *                           Leading zeros are allowed.
 *
  * @return          
 *                  - ESP_OK : Success, the passkey reply was sent successfully.
 *                  - ESP_FAIL : Failed, the passkey reply could not be sent. Refer to the error codes for details.
 *
 * @note            This function ralated to the following event:
 *                  - `ESP_GAP_BLE_AUTH_CMPL_EVT`: Indicates that the authentication process has completed, 
 *                    which may include the result of the passkey entry.
 *                  - `ESP_GAP_BLE_PASSKEY_REQ_EVT`: Triggered when a pairing code request is sent, notifying the application to enter a pairing code.
 */
esp_err_t esp_ble_passkey_reply(esp_bd_addr_t bd_addr, bool accept, uint32_t passkey);


/**
 * @brief           Reply with the confirmation value to the peer device during the secure connection stage.
 *                  This function is used to respond to a numeric comparison request from the peer device.
 *
 * @param[in]       bd_addr : The Bluetooth Device address of the peer device. This is a 6-byte address
 *                            that uniquely identifies the Bluetooth device.
 * @param[in]       accept  : Indicates whether the numbers to compare are the same or different.
 *                            Set to `true` if the numbers are the same, otherwise `false` if they are different.
 *
  * @return          
 *                  - ESP_OK : Success, the confirmation reply was sent successfully.
 *                  - ESP_FAIL : Failed, the confirmation reply could not be sent. Refer to the error codes for details.
 * 
 * @note            This function may trigger the following event:
 *                  - ` ESP_GAP_BLE_NC_REQ_EVT`: This event is triggered when a numeric comparison request is received from the peer device.
 */
esp_err_t esp_ble_confirm_reply(esp_bd_addr_t bd_addr, bool accept);

/**
 * @brief           Removes a device from the security database list of peer devices.
 *                  This function manages the unpairing event while connected.
 *                  It effectively unbonds the local device from the peer device, removing
 *                  any stored security information such as keys and pairing status.
 *
 * @param[in]       bd_addr : The Bluetooth Device address of the peer device. This is a 6-byte address
 *                            that uniquely identifies the Bluetooth device.
 *
  * @return          
 *                  - ESP_OK : Success, the device was removed from the security database successfully.
 *                  - ESP_FAIL : Failed, the device could not be removed. Refer to the error codes for details.
 *
 * @note            This function may trigger the following event:
 *                  - `ESP_GAP_BLE_REMOVE_BOND_DEV_COMPLETE_EVT`: Indicates that the removal of the bonded device has been processed.
 */
esp_err_t esp_ble_remove_bond_device(esp_bd_addr_t bd_addr);

/**
 * @brief           Get the number of devices from the security database list of peer devices.
 *                  This function returns the number of bonded devices immediately.
 *                  It queries the local security database and counts how many devices have been bonded with.
 *
  * @return          
 *                  - >= 0 : The number of bonded devices.
 *                  - ESP_FAIL : Failed to retrieve the number of bonded devices.
 */
int esp_ble_get_bond_device_num(void);

/**
 * @brief           Get the list of bonded devices from the security database of peer devices.
 *                  This function returns the bonded devices' information immediately.
 * 
 ...
 *                           It is recommended that the `dev_num` value is equal to the value returned by `esp_ble_get_bond_device_num()`.
 * 
...
/**
 * @brief           This function is called to provide the Out of Band (OOB) data for
 *                  Security Manager Protocol (SMP) in response to `ESP_GAP_BLE_OOB_REQ_EVT` event.
 *                  OOB data is used during pairing to ensure secure transmission of the Temporary Key (TK).
 *
 ...
 *                  - ESP_FAIL : Failed
 *
 * @note            This function is called in response to the event:
 *                  - `ESP_GAP_BLE_OOB_REQ_EVT`: Indicates that the peer device is requesting OOB data.
 */
esp_err_t esp_ble_oob_req_reply(esp_bd_addr_t bd_addr, uint8_t *TK, uint8_t len);

/**
 * @brief           This function is called to provide the Out of Band (OOB) data for
 *                  Security Manager Protocol (SMP) in response to the `ESP_GAP_BLE_SC_OOB_REQ_EVT` event.
 *                  OOB data is used during Secure Connections (SC) pairing to ensure secure transmission
 *                  of the Confirmation value and the Random value.
 *
 * @param[in]       bd_addr: Bluetooth Device Address (BD_ADDR) of the peer device.
 ...
 * 
 * @note            This function is called in response to the event:
 *                  - `ESP_GAP_BLE_SC_OOB_REQ_EVT`: Indicates that the peer device is requesting Secure Connections OOB data.
 */
esp_err_t esp_ble_sc_oob_req_reply(esp_bd_addr_t bd_addr, uint8_t p_c[16], uint8_t p_r[16]);

/**
 * @brief           This function is called to create the Out of Band (OOB) data for
 *                  Security Manager Protocol (SMP) during Secure Connections (SC).
 *                  The OOB data generated by this function is used for pairing with devices
 *                  using Secure Connections, providing an additional layer of security
 *                  by allowing the exchange of cryptographic information out of the Bluetooth
 *                  communication channel.
 * 
 *                  Throughout the pairing procedure, the `ESP_GAP_BLE_SC_OOB_REQ_EVT` is triggered to request peer SC OOB data. 
 *                  The SC OOB data is generated using the API `esp_ble_create_sc_oob_data` and is displayed in the `ESP_GAP_BLE_SC_CR_LOC_OOB_EVT` on the peer device.
 *
  * @return          
 *                  - ESP_OK : Success
 *                  - ESP_FAIL : Failed
 * 
 * @note            This function is called in response to the event:
 *                  - ` ESP_GAP_BLE_SC_CR_LOC_OOB_EVT`: This event indicates that the local device's SC OOB data creation is complete.
 */
...
/**
 * @brief           This function is used to disconnect the physical Bluetooth connection 
 *                  with a peer device. This function directly terminates the Bluetooth link 
 *                  between the local device and the specified peer device address. 
 *                  Unlike `esp_ble_gattc_close` which only closes one virtual GATT server 
 *                  connection (if multiple connections are established), this function 
 *                  ensures that the entire physical connection is disconnected.
 *
 * @param[in]       remote_device: The Bluetooth Device Address (BD_ADDR) of the peer device 
 *                  to be disconnected. This is a 6-byte address that uniquely identifies the 
 *                  peer device in Bluetooth communication.
 *
  * @return          
 *                  - ESP_OK : Success, the physical connection was successfully disconnected.
 *                  - ESP_FAIL : Failed, the disconnection attempt failed. Refer to the error codes 
 *                            for details on the failure.
 *
 * @note            This function is used to terminate the connection at the physical level, 
 *                  which means all virtual GATT server connections associated with the peer 
 *                  device will also be terminated. If there are multiple virtual GATT server 
 *                  connections, `esp_ble_gattc_close` can be used to close a specific GATT 
 *                  connection, but it does not affect the physical connection.
 * 
 *                 This function may trigger the following event:
 *                  - `ESP_GATTS_DISCONNECT_EVT`: This event indicates that a GATT server has disconnected from a peer device.
 */
esp_err_t esp_ble_gap_disconnect(esp_bd_addr_t remote_device);

/**
 * @brief           This function retrieves the current connection parameters of a Bluetooth device.
 *                  It provides information about the connection parameters for a given peer device.
 *                  Connection parameters include interval, latency, and supervision timeout,
 *                  which are crucial for managing and optimizing Bluetooth connections.
 *
 * @param[in]       bd_addr: The Bluetooth Device Address (BD_ADDR) of the peer device.
 *                  This is a 6-byte address that uniquely identifies the peer device in Bluetooth communication.
 * @param[out]      conn_params: Pointer to an `esp_gap_conn_params_t` structure where the connection 
 *                  parameters information will be stored. The structure will be filled with the current 
 *                  connection parameters of the specified peer device.
 *
  * @return          
 *                  - ESP_OK : Success, the connection parameters were successfully retrieved.
 *                  - ESP_FAIL : Failed, the attempt to retrieve connection parameters failed. 
...
/**
 * @brief           This function is used to set the channels for Bluetooth Low Energy (BLE) communication.
 *                  The function allows configuring the link layer channel indices to indicate their status.
 *                  Channels can be marked as "bad", "unknown", or reserved.
 *
 * @param[in]       channels: A bit field where each bit represents the status of a BLE channel.
 ...
 *
  * @return          
 *                  - ESP_OK : Success, channels were set successfully.
 *                  - ESP_ERR_INVALID_STATE : failed, Bluetooth stack is not yet enabled.
 *                  - ESP_FAIL : Failed, the attempt to set channels failed. Refer to the error codes for details.
 *
 * @note            The function will not trigger any specific events. It directly configures the channels
 *                  based on the provided `channels` bit field.
 *                  It is essential that at least one channel is marked as "unknown" to ensure proper operation.
 *                  Ensure that the Bluetooth stack is enabled before calling this function to avoid
 *                  `ESP_ERR_INVALID_STATE` errors.
 *
 *                  This function may trigger the following event:
 *                  - `ESP_GAP_BLE_SET_CHANNELS_EVT`: Triggered when setting up the BLE channel is complete, notifying the application of the result of the setup.
 */
esp_err_t esp_gap_ble_set_channels(esp_gap_ble_channels channels);

/**
 * @brief           This function is called to authorize a link after authentication, providing Man-in-the-Middle (MITM) protection.
 ...
 * @param[in]       authorize: `true` if authorize the link, allowing it to proceed; otherwise `false` to deny authorization, 
 *                  preventing further communication with the peer device.
 *
  * @return          
 *                  - ESP_OK : Success, the authorization decision was processed successfully.
 *                  - ESP_FAIL : Failed, the authorization decision could not be processed. Refer to the error codes for details.
 */

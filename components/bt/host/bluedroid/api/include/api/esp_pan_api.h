...
/**
 * @brief Ethernet MAC address length used by PAN data and multicast filters.
 */
#define ESP_PAN_MAC_ADDR_LEN            6

/**
 * @brief Ethernet MAC address type used by PAN.
 *
 * Distinct from esp_bd_addr_t, which is a Bluetooth device address.
 */
typedef uint8_t esp_pan_mac_addr_t[ESP_PAN_MAC_ADDR_LEN];
...
typedef union {
    ...
    struct pan_data_ind_evt_param {
        ...
        esp_pan_mac_addr_t src;           /*!< Source Ethernet MAC address of the received frame. */
        esp_pan_mac_addr_t dst;           /*!< Destination Ethernet MAC address of the received frame. */
 ...
 *
 * @param[in]   handle:     Connection handle obtained from ESP_PAN_OPEN_EVT.
 * @param[in]   dst:        Destination Ethernet MAC address.
 *                          Use broadcast address for ARP requests.
 * @param[in]   src:        Source Ethernet MAC address.
 *                          Usually the local PAN netif MAC address.
 ...
 *
 * @note        dst and src are Ethernet MAC addresses, not Bluetooth device addresses.
 ...
 *                              Pass 0 with NULL arrays to clear filters.
 * @param[in]   start_array:    Array of range start Ethernet MAC addresses. Ignored when num_filters is 0.
 * @param[in]   end_array:      Array of range end Ethernet MAC addresses. Ignored when num_filters is 0.
 *
 * @return
 *              - ESP_OK: success
 *              - ESP_ERR_INVALID_ARG: invalid handle, NULL arrays when num_filters > 0, or too many filters

...
#else
#define ESP_BOOTLOADER_RESERVE_RTC (CONFIG_BOOTLOADER_SYSTEM_RESERVE_RTC_SIZE)
#endif

_Static_assert(CONFIG_BOOTLOADER_SYSTEM_RESERVE_RTC_SIZE % 4 == 0, "CONFIG_BOOTLOADER_SYSTEM_RESERVE_RTC_SIZE must be a multiple of 4 bytes");
_Static_assert(sizeof(rtc_retain_mem_t) == ESP_BOOTLOADER_RESERVE_RTC, "Reserved RTC area must equal size of rtc_retain_mem_t");

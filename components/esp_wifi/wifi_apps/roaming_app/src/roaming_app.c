...
static const u8 *roaming_app_wpa_bss_get_ie(const uint8_t *bssid, u8 elem_id)
{
    struct wpa_bss *bss;
    ...
    bss = wpa_bss_get_bssid(&g_wpa_supp, bssid);
    if (!bss) {
        return NULL;
    }

    return wpa_bss_get_ie(bss, elem_id);
}
...
static const u8 *roaming_app_get_rsnxe_ie(const uint8_t *bssid)
{
    ...
    return rsnxe ? (const u8 *) rsnxe : NULL;
}
...
static void roaming_app_apply_low_rssi_config(void)
{
    ...
    if (esp_wifi_set_rssi_threshold(next_threshold) == ESP_OK) {
        ...
        ESP_LOGW(ROAMING_TAG, "failed to set low RSSI threshold to %d", (int)next_threshold);
    }
}
#endif /*LOW_RSSI_ROAMING_ENABLED*/
...
static void roaming_app_rssi_low_internal_handler(void *ctx, void *data)
{
    ...
    if (esp_wifi_set_rssi_threshold(next_threshold) == ESP_OK) {
        ...
        ESP_LOGW(ROAMING_TAG, "failed to reset RSSI threshold to %d", (int)next_threshold);
    }
...
{
    const u8 *rsn_ie = roaming_app_wpa_bss_get_ie(candidate.bssid, WLAN_EID_RSN);

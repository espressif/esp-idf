...
void esp_efuse_utility_debug_dump_pending()
{
    for (int num_block = 0; num_block < EFUSE_BLK_MAX; num_block++) {
        int subblock = 0;
        for (uint32_t addr_wr_block = range_write_addr_blocks[num_block].start; addr_wr_block <= range_write_addr_blocks[num_block].end; addr_wr_block += 4) {
            if (virt_blocks[num_block][subblock++] != REG_READ(addr_wr_block)) {
                esp_efuse_utility_debug_dump_single_block(num_block);
                break; // one log per block
            }
        }
    }
}
...
bool esp_efuse_utility_load_efuses_from_flash(void)
{
    ...
    ESP_EARLY_LOGW(TAG, "[Virtual] no fuses found in flash");
...
void esp_efuse_utility_write_efuses_to_flash(void)
{
    ...
    if (err != ESP_OK) {
        ESP_EARLY_LOGE(TAG, "[Virtual] Failed to write eFuses to flash. Bad secure_version. err = 0x%x ", err);
        abort();
    }
}
#endif // CONFIG_EFUSE_VIRTUAL_KEEP_IN_FLASH

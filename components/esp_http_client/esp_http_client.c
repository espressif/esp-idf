...
static int http_on_headers_complete(http_parser *parser)
{
    esp_http_client_handle_t client = parser->data;
    http_on_header_event(client);
    http_dispatch_event(client, HTTP_EVENT_ON_HEADERS_DONE, NULL, 0);
    http_dispatch_event_to_event_loop(HTTP_EVENT_ON_HEADERS_DONE, &client, sizeof(esp_http_client_handle_t));

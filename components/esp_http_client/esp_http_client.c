...
esp_http_client_handle_t esp_http_client_init(const esp_http_client_config_t *config)
{
    ...
    if (config->use_global_ca_store == true) {
        ...
        if (!config->cert_len) {
            ...
            esp_transport_ssl_set_cert_data(ssl, config->cert_pem, config->cert_len);
        }
    }
    ...
    if (config->client_cert_pem) {
        if (!config->client_cert_len) {
          ...
          esp_transport_ssl_set_client_cert_data(ssl, config->client_cert_pem, config->client_cert_len);
        }
    }
    ...
    if (config->client_key_pem) {
        if (!config->client_key_len) {
            ...
            esp_transport_ssl_set_client_key_data(ssl, config->client_key_pem, config->client_key_len);
        }
    }

...
#define NimBLE_AMS_TAG  "NimBLE_AMS"
...
static void log_playback_info(const uint8_t *value, uint16_t value_len)
{
    ...
    if (state[0] != '\0') {
        ...
        ESP_LOGI(NimBLE_AMS_TAG, "  PlaybackState: %s", PlaybackState_to_String((uint8_t)playback_state));
    }
    if (rate && rate[0] != '\0') {
        ESP_LOGI(NimBLE_AMS_TAG, "  PlaybackRate: %s", rate);
    }
    if (elapsed && elapsed[0] != '\0') {
        ESP_LOGI(NimBLE_AMS_TAG, "  ElapsedTime: %s s", elapsed);
    }
}
...
void ble_receive_remote_command_notification(uint8_t *message, uint16_t message_len)
{
    ...
    ESP_LOGI(NimBLE_AMS_TAG, "Supported remote commands (%u):", message_len);
    for (uint16_t i = 0; i < message_len; i++) {
        ESP_LOGI(NimBLE_AMS_TAG, "  [%u] %s", i, RemoteCommandID_to_String(message[i]));
    }
}
...
void ble_receive_entity_update(uint8_t *message, uint16_t message_len)
{
    ...
    ESP_LOGI(NimBLE_AMS_TAG, "Entity update: %s / %s, flags=0x%02x, len=%u",
    ...
    if (flags & EntityUpdateFlagTruncated) {
        ESP_LOGI(NimBLE_AMS_TAG, "Value truncated; read full value from Entity Attribute characteristic");
    }
}
...
{
    ESP_LOGI(NimBLE_AMS_TAG, "Full entity attribute: %s / %s, len=%u",
             EntityID_to_String(entity_id),
             AttributeID_to_String(entity_id, attribute_id),
             message_len);

...
#define RESTART_TIME_OUT    60000 * 1000    // 60s
#define NEXT_PROV           5000 * 1000     // 5s
static esp_timer_handle_t  node_timer;
                                             ...
                                             esp_ble_mesh_prov_cb_param_t *param)
{
    ...
    case ESP_BLE_MESH_NODE_PROV_LINK_OPEN_EVT:
        esp_timer_stop(node_timer);
        int ret = esp_timer_start_once(node_timer, RESTART_TIME_OUT );
        if (ret != ESP_OK) {
    ...
    case ESP_BLE_MESH_NODE_PROV_RESET_EVT:{
        ...
        esp_timer_stop(node_timer);
        int ret = esp_timer_start_once(node_timer, NEXT_PROV );
        if (ret != ESP_OK) {
            ESP_LOGE(TAG, "timer start failed %d %x\n",__LINE__,ret);
            return;
        }
...
esp_err_t create_prov_timer( esp_timer_handle_t * handle)
{
    ...
    ret = esp_timer_start_once(*handle, RESTART_TIME_OUT );
    if (ret != ESP_OK) {
...
void IRAM_ATTR btdm_scan_trigger_gpio()
{

}
void IRAM_ATTR btdm_adv_trigger_gpio()
{
...
void app_main(void)
{
    ...
    err = create_prov_timer(&node_timer);
    if (err != ESP_OK) {

...
extern void dbg_cnt_lmac_eb_show(void);
extern void dbg_cnt_lmac_q_show(void);
void iperf_report_task(void* arg)
{
    ...
    while (!s_iperf_ctrl.finish) {
        ...
        if (s_iperf_ctrl.total_len == last_len) {
...
esp_err_t iperf_start_report(void)
{
    ...
    ret = xTaskCreatePinnedToCore(iperf_report_task, IPERF_REPORT_TASK_NAME, IPERF_REPORT_TASK_STACK, NULL, IPERF_REPORT_TASK_PRIORITY, NULL, 0);
...
esp_err_t iperf_start(iperf_cfg_t *cfg)
{
    ...
    ret = xTaskCreatePinnedToCore(iperf_task_traffic, IPERF_TRAFFIC_TASK_NAME, IPERF_TRAFFIC_TASK_STACK, NULL, IPERF_TRAFFIC_TASK_PRIORITY, NULL, 0);
    if (ret != pdPASS)  {

...
def test_Bidirectional_IPv6_connectivity(Init_interface:bool, dut: Tuple[IdfDut, IdfDut, IdfDut]) -> None:
    ...
    try:
        ...
        for src_addr in host_global_unicast_addr:
            ...
            print('ping parsed result (source=%s): loss_rate=%d%%', src_addr, loss_rate)
        print('host_ping_loss_rates: %s', host_ping_loss_rates)
...
def test_ot_ssed_device(dut: Tuple[IdfDut, IdfDut]) -> None:
    ...
    try:
        ...
        ocf.ping_and_check(dut=leader, target=ssed_address, tx_total=10, timeout=6, size_range=(10, 200))

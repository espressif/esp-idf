        ...
        def wrapper(dut: IdfDut) -> str:
            ...
            for attempt in range(1, retries + 1):
                try:
                    ...
                    print(f'[{command}] Attempt {attempt}/{retries} failed: {e}')
                    last_exception = e
                    if attempt < retries:
                        time.sleep(delay)

            if last_exception:
                print(f'[{command}] Giving up after {retries} retries.')
...
def wait_for_host_onlink_global_address(
    ...
    *,
    ...
    print(f'Wait for host GUA in BR onlink prefix {onlinkprefix!r} on {interface_name}')
...
def ensure_avahi_running(restart_if_needed: bool = True) -> bool:
    ...
    if out_str.strip():
        print(f'avahi process list:\n{out_str}')
        return True

    print('avahi-daemon not running')
    if restart_if_needed:
        print('restarting avahi-daemon once...')
        restart_avahi()
        out_str = subprocess.getoutput('pgrep -a avahi-daemon 2>/dev/null')
        if out_str.strip():
            print(f'avahi process list after restart:\n{out_str}')
            return True

    print('avahi-daemon is still not running')
    return False
...
def get_domain() -> str:
    ...
    print(f'avahi status:\n{out_str}')
    matches = re.findall(r'\[([\w\W]+?)\.local\]', str(out_str))
    if matches:
        role = matches[0]
        print(f'active host is: {role}')
        return str(role)
...
def log_ipv6_addr_route_by_interface(interface_name: str, title: str = '') -> Tuple[str, str]:
    ...
    print(f'{prefix}IPv6 addresses on {interface_name}:\n{addr}')
    print(f'{prefix}IPv6 routes on {interface_name}:\n{route}')
    return addr, route


def flush_ipv6_addr_route_by_interface(interface_name: str, down_up_wait_s: int = 5) -> None:
    print(f'flush ipv6 addr/route: {interface_name}')
...
def wait_for_host_network(host: str = '8.8.8.8', retries: int = 6, interval: int = 10) -> None:
    for attempt in range(1, retries + 1):
        try:
            ...
            print(f'Host network reachable on attempt {attempt}')
            return
        except subprocess.CalledProcessError:
            print(f'Ping attempt {attempt} failed, retrying in {interval} seconds...')
            if attempt < retries:
                time.sleep(interval)
            else:

...
static esp_err_t submit_async_req(httpd_req_t *req, httpd_req_handler_t handler)
{
    ...
    // How show we handle resource exhastion?
    // In this example, we immediately respond with an
    // http error if no workers are available.
    //
    // Note: If you set a value higher than 0, it will block the
    // httpd thread for longer, which will make your server less
...
static esp_err_t long_async_handler(httpd_req_t *req)
{
    ...
    if (is_on_async_worker_thread() == false) {
        ...
        if (submit_async_req(req, long_async_handler) == ESP_OK) {
            // claim ownership. This signals to the server to not
            // purge the socket used for this request until
            // async work has completed and we relinquish ownership.
            // Note: this is necessary because lru_purge_enable = true
...
void async_req_worker_task()
{
    ...
    while(true) {
        ...
        if (xQueueReceive(asyncReqQueue, &asyncReq, portMAX_DELAY)) {
            ...
            if(httpd_req_relinquish_ownership(asyncReq.req) != ESP_OK) {
                ESP_LOGE(TAG, "failed to relinquish ownerships");
            }

            // free our request copy
            httpd_req_free_copy(asyncReq.req);
        }
    }

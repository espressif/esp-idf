...
#define BATTRY_CHAR_CONFIG_DESC 0x2902
...
static int battery_descriptor(uint16_t conn_handle, uint16_t attr_handle, struct ble_gatt_access_ctxt *ctxt, void *arg)
{
     ...
     .characteristics = (struct ble_gatt_chr_def[]){
         {.uuid = BLE_UUID16_DECLARE(BATTERY_LEVEL_CHAR),
          ...
          .descriptors = (struct ble_gatt_dsc_def[]){
              {
                  .uuid = BLE_UUID16_DECLARE(BATTRY_CHAR_CONFIG_DESC),
...
void app_main(void)
{
    ...
    timer_handler = xTimerCreate("update_battery_status",pdMS_TO_TICKS(1000), pdTRUE,NULL,update_battery_status);

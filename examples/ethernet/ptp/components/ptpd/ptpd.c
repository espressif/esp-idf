...
#ifdef CONFIG_NETUTILS_PTPD_GPTP_PROFILE
      || ((a->stepsremoved[0] << 8) | a->stepsremoved[1]) <
             ((b->stepsremoved[0] << 8) | b->stepsremoved[1]))
#endif
  {
    return true;
  } else {
...
#else
  state->own_identity.gm_priority1 =
      255; // When daemon is statically configured as slave, set the worst
#endif
...
#ifdef ESP_PTP
  ret = ptp_net_send(state, &msg, sizeof(struct ptp_delay_req_s),
                     &state->delayreq_time);
#else
...
static int ptp_periodic_send(FAR struct ptp_state_s *state) {
  ...
  if (!state->selected_source_valid) {
    ...
    clock_gettime(CLOCK_MONOTONIC, &time_now);
    clock_timespec_subtract(&time_now, &state->last_transmitted_announce,
                            &delta);
    ...
    defined(                                                                   \
        CONFIG_NETUTILS_PTPD_GPTP_PROFILE) // gPTP always sends delay requests
  if (state->selected_source_valid && state->can_send_delayreq) {
    struct timespec time_now;
    struct timespec delta;

    clock_gettime(CLOCK_MONOTONIC, &time_now);
    clock_timespec_subtract(&time_now, &state->last_transmitted_delayreq,
                            &delta);
...
static void ptp_lock_local_clock_freq(FAR struct ptp_state_s *state,
  ...
  // Compute difference between number of ticks in slave and master over sync
  // period. This is used to lock the frequency with the master. However, it
  // never catch-up the offset by itself, hence also add `adj` at the end
  ...
  // compute how to scale the slave frequency to lock with master frequency and
  // also try to catch-up the offset
  double freq_scale = ((double)(remote_delta_ns /*+ tick_diff*/ + adj)) /
                      (double)local_delta_ns;
  esp_eth_clock_adj_param_t clk_adj_param = {.mode = ETH_CLK_ADJ_FREQ_SCALE,
  ...
  // Get the path delay only when clock is stable enough. If we were in process
  // of adjustion (speeding/slowing slave), we would get incorrect delay
...
static int ptp_process_delay_req(FAR struct ptp_state_s *state,
  ...
  ret = ptp_net_send(state, &msg, sizeof(struct ptp_delay_resp_follow_up_s),
                     NULL);
  if (ret < 0) {
...
static int ptp_process_delay_resp(FAR struct ptp_state_s *state,
  ...
  if (path_delay >= 0 && path_delay < CONFIG_NETUTILS_PTPD_MAX_PATH_DELAY_NS) {
    ...
    state->path_delay_ns +=
        (path_delay - state->path_delay_ns) / state->path_delay_avgcount;

    ptpinfo("Path delay: %ld ns (avg: %ld ns)\n", (long)path_delay,
            (long)state->path_delay_ns);
  } else {
...
static int ptp_process_rx_packet(FAR struct ptp_state_s *state,
  ...
  case PTP_MSGTYPE_SYNC:
    ...
    if (!state->selected_source_valid) {
      return OK;
    } // ignore if operating as a server
...
#ifdef ESP_PTP
      ret = ptp_net_recv(state, &state->rxbuf, sizeof(state->rxbuf),
                         &state->rxtime);
#else

...
However, an alternative mode :menuitem:`CONFIG_SECURE_BOOTLOADER_MODE` is also available. This mode allows you to supply a binary key file that is used for the secure bootloader key. As you have the key file, you can generate new bootloader images and secure boot digests for them.
...
1. Enable :menuitem:`CONFIG_SECURE_BOOT`, select :menuitem:`CONFIG_SECURE_BOOT_V1_ENABLED`, and set :menuitem:`CONFIG_SECURE_BOOTLOADER_MODE` to ``Reflashable``.

2. If necessary, set the :menuitem:`CONFIG_SECURE_BOOTLOADER_KEY_ENCODING` based on the coding scheme used by the device. The coding scheme is shown in the ``Features`` line when ``esptool`` connects to the chip, or in the ``idf.py efuse-summary`` output.
...
1. Enable :menuitem:`CONFIG_SECURE_SIGNED_APPS_NO_SECURE_BOOT`.

...
- ``IDF_CLONE_SHALLOW_DEPTH``: If this argument is set to a non-empty value and ``IDF_CLONE_SHALLOW`` is set to a non-empty value too, ``--depth=X`` can be specifyed with a given number X to clone a specific depth during shallow clone. This helps to enable the ``git describe`` command reporting a valid version number on branches with a commit offset since the last tag. If using a branch with commit offset, run ``git describe`` in your ESP-IDF Repository, note the offset number and increment it by 1, then use it here to create an image with working verison number of the ESP-IDF version.

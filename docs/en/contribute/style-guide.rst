...
It is possible to configure ESP-IDF projects with assertions disabled (see :menuitem:`CONFIG_COMPILER_OPTIMIZATION_ASSERTION_LEVEL`). Therefore, functions called in an ``assert()`` statement should not have side-effects.

...
CCache_ 是编译器缓存，通过缓存先前的编译结果来加速重新编译。启用后，当使用相同的源代码和编译器标志重复编译时，CCache 会直接返回缓存结果，而无需重新编译。
...
重要：源文件不会被修改，仍然包含 ``#include <sdkconfig.h>``，并且无需更改源代码中的任何内容，configdep 工具即可正常工作。该工具完全基于生成的依赖文件 (``.d``) 进行工作。
...
多个加速工具被启用时，会作为编译器启动器被串联起来，并按特定顺序应用：

1. **esp-idf-configdep** （若启用） - 处理依赖文件并优化由 ``sdkconfig.h`` 变更引起的重建
2. **ccache** （若启用） - 缓存编译结果

链中的每个工具都会嵌套调用下一个工具，最终调用实际的编译器。这样可以叠加多个工具的优势。然而，组合使用多个工具并不总能带来最佳性能，可能需要尝试不同的工具组合，从而让项目达到最佳的构建性能。

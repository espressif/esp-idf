...
- ``目标`` 特指运行构建后应用程序的硬件设备。ESP-IDF 当前仅支持 ``esp32`` 和 ``esp32s2beta`` 这两个硬件目标。
...
更多信息，请参考文件 :idf_file:`/tools/cmake/project.cmake` 以及 :idf_file:`/tools/cmake/idf_functions.cmake` 中支持的函数。
...
* ``esp32s2beta``— 适用于 ESP32-S2-beta（工程样品）
...
您也可以将要用的 ``IDF_TARGET`` 设置为环境变量，比如：``export IDF_TARGET=esp32s2beta``；或设置为 CMake 变量，比如将 ``-DIDF_TARGET=esp32s2beta`` 以参数形式传递给 CMake 或 idf.py。如果您大多数时间仅使用一款芯片，则将 ``IDF_TARGET`` 配置为环境变量比较方便。

对于特定项目，您可以使用以下方式为 ``IDF_TARGET`` 配置 _default_ 值：把 ``CONFIG_IDF_TARGET`` 的值加入 ``sdkconfig.defaults``。举例而言，配置 ``CONFIG_IDF_TARGET="esp32s2beta"``。这样一来，除非特别设置（比如使用环境变量、CMake 变量或 ``idf.py set-target`` 命令），否则 ``IDF_TARGET`` 将默认采用 ``CONFIG_IDF_TARGET``。

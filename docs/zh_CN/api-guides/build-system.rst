...
通用组件包括：freertos, newlib, heap, log, soc, esp_rom, esp_common, xtensa, cxx。
...
- SDKCONFIG_DEFAULTS - 默认用于构建的文件，默认为空；
...
- SDKCONFIG_DEFAULTS - 配置默认文件的完整路径；由 ``idf_build_process`` SDKCONFIG_DEFAULTS 参数设置。

...
- flash 文件大小与项目配置中 :menuitem:`CONFIG_ESPTOOLPY_FLASHSIZE` 的指定值相同。
...
QEMU 支持模拟安全启动 v2 机制。请保持 :menuitem:`CONFIG_SECURE_BOOT_FLASH_BOOTLOADER_DEFAULT` 处于启用状态，将签名的引导加载程序镜像嵌入到 QEMU 的镜像文件中。

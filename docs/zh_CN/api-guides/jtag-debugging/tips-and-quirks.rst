...
* ``mon reset halt`` — 复位芯片并使 CPU 停止运行。必须复位目标芯片以禁用内存保护，这是启用 flash 支持所必需的。如果不希望复位，可在 OpenOCD 命令行开头添加额外参数 ``-c 'set ESP_FLASH_SIZE 0'`` 来禁用 flash 支持，详见 :ref:`jtag-debugging-tip-openocd-config-vars`。另一种方法是使用配置选项 :ref:`CONFIG_ESP_SYSTEM_MEMPROT_FEATURE` 来禁用内存保护。

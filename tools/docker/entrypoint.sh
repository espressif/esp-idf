...
# All paths specified in IDF_GIT_SAFE_DIR will be added to user's
# global git config as safe.directory paths. For more information
# see git-config manual page.
if [ -n "${IDF_GIT_SAFE_DIR+x}" ]; then
...
# Toggle exec behavior based on EXEC_IN_SAME_SHELL environment variable
if [ "$EXEC_IN_SAME_SHELL" = "true" ]; then
    "$@"
else
    exec "$@"
fi

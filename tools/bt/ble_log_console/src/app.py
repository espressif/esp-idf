...
class BLELogApp(App):
    ...
    def on_mount(self) -> None:
        if self._port is not None:
            ...
            self.push_screen(
                LaunchScreen(default_log_dir=self._log_dir),
    ...
    def _emit_stats(
        self, stats: StatsAccumulator, parser: FrameParser, last_time: float
    ) -> float:
    ...
    def _backend_loop(self) -> None:
        ...
        console_log_path = self._output_path.with_name(
            self._output_path.stem + '_console.log'
        )
        ...
        try:
            while True:
                ...
                if not block:
                    last_snapshot_time = self._emit_stats(
                        stats, parser, last_snapshot_time
                    )
                    continue
                ...
                for item in results:
                    if isinstance(item, ParsedFrame):
                        ...
                        if item.source_code != BleLogSource.INTERNAL:
                            stats.record_frame(
                                frame_size, item.source_code, item.frame_sn
                            )
                        ...
                        if (
                            has_os_ts(item.source_code)
                            and item.source_code != BleLogSource.INTERNAL
                        ):
                            stats.record_frame_ts(
                                item.os_ts_ms, frame_size, item.source_code
                            )
                        elif is_ll_source(item.source_code) and len(item.payload) >= 6:
                            (lc_ts_us,) = struct.unpack_from(
                                '<I', item.payload, LL_TS_OFFSET
                            )
                            stats.record_ll_frame_ts(
                                lc_ts_us, frame_size, item.source_code
                            )
                        elif item.source_code == BleLogSource.REDIR:
                            wall_ms = int(time.perf_counter() * 1000) & 0xFFFFFFFF
                            stats.record_frame_wall_ts(
                                wall_ms, frame_size, item.source_code
                            )
                        ...
                        # batch sealing, so buffer partial lines until '\n'.
                            ...
                            payload_text = item.payload.decode(
    ...
    def on_internal_frame_decoded(self, msg: InternalFrameDecoded) -> None:
        if msg.int_src == InternalSource.INIT_DONE:
            ...
            log_view = self.query_one(LogView)
            log_view.write_info(
                f"BLE Log v{info['version']} initialized - statistics reset"
            )
    ...
    def on_traffic_spike_detected(self, msg: TrafficSpikeDetected) -> None:
        ...
        src_parts = ', '.join(
            f'{resolve_source_name(s)} {p:.0f}%' for s, p in top_sources if p >= 1.0
        )
        ...
        log_view.write_traffic(
            f'{msg.throughput_kbs:.0f} KB/s ({util_str}) over {msg.duration_ms:.0f}ms | {src_parts}'
        )

...
class MemberField:
    ...
    def __str__(self) -> str:
        if self.bitfield is None:
            return f'"Field type={self.member_type}"'
        return f'"Field type={self.member_type} bit={self.bitfield}"'
...
class SoCStructureHeaderChecker:
    ...
    def __getline(self, incomment: bool = False) -> Any:
        ...
        if rawline.count(';') > 1:
            print(f'\033[0;34mINFO\033[0m: line: {self.__linecount}: possibly multiple expression within same line')
            print(rawline)
        return rawline
    ...
    def __process_structure(self, name: str, is_typedef: bool, is_volatile: bool) -> Any:
        ...
        if is_typedef and is_volatile and name is None:
            print(f'\033[0;31mERROR\033[0m: line {self.__linecount}: anonymous struct')
            ret_val = -1
        node_tree = dict()
        bitcount = 0
        ...
        while not self.__is_eof:
            ...
            if match_obj is not None:
                ...
                if bitcount not in (0, 32):
                    ...
                    if is_typedef:
                        print(
                            f'\033[0;31mERROR\033[0m: line {self.__linecount}: bitfield count is {bitcount}, '
                            f'type {match_obj.groups()[0]}'
                        )
                    else:
                        print(
                            f'\033[0;31mERROR\033[0m: line {self.__linecount}: bitfield count is {bitcount}, '
                            f'type {name}, varname "{match_obj.groups()[0]}"'
                        )
                ...
                if is_typedef:
                    ...
                    if match_obj.groups()[0] == '' or match_obj.groups()[0].find('[') != -1:
                        # should be c error
                        print(f'\033[0;31mERROR\033[0m: line {self.__linecount}: C error')
                        ret_val = -3
            ...
            if match_obj is not None:
                ...
                if match_obj.groups()[2] is not None:
                    ...
                    if match_obj.groups()[0] != 'uint32_t':
                        print(
                            f'\033[0;33mWARN\033[0m: line: {self.__linecount}: {match_obj.groups()[1]} '
                            f'has type {match_obj.groups()[0]}'
                        )
                else:
                ...
                if member_node is not None:
                    ...
                    if '*' not in match_obj.groups()[0]:
                        print(f'\033[0;33mWARN\033[0m: line {self.__linecount}: unknown type {match_obj.groups()[0]}')
                    else:
                        print(f'\033[0;33mWARN\033[0m: line {self.__linecount}: pointer type {match_obj.groups()[0]}')
                continue
            # check comments
            ...
            print(
                (f'\033[0;33mWARN\033[0m: line: {self.__linecount}: unexpected expression: {rawline}').replace('\n', '')
            )

        if bitcount != 0 and has_nested_struct_union:
            print(f'\033[0;33mWARN\033[0m: line: {self.__linecount}: mixed bitfield member and nested structure/union')
        if bitcount != 0 and has_non_bitfield_member:
            print(f'\033[0;33mWARN\033[0m: line: {self.__linecount}: mixed bitfield member and non-bitfield member')
        if is_typedef and is_volatile and name is None:
            if parsed_varname != '':
        ...
        if name is not None and is_typedef and is_volatile and parsed_varname.rstrip('t') != name.rstrip('s'):
            print(
                f'\033[0;33mWARN\033[0m: line: {self.__linecount}: different type and typedef name: '
                f'{name} {parsed_varname}'
            )
    ...
    def __process_union(self, name: str, is_typedef: bool, is_volatile: bool) -> Any:
        ...
        # first check for anonymous register structs
        if is_typedef and is_volatile and name is None:
            print(f'\033[0;31mERROR\033[0m: line {self.__linecount}: anonymous union')
            ret_val = -1
        ...
        while not self.__is_eof:
            ...
            if match_obj is not None:
                ...
                if match_obj.groups()[0] != 'uint32_t' or match_obj.groups()[1] != 'val':
                    print(
                        (f'\033[0;33mWARN\033[0m: unexpected union member at {self.__linecount}: {rawline}').replace(
                            '\n', ''
                        )
                    )
                else:
                ...
                if member_node is not None:
                    ...
                    if '*' not in match_obj.groups()[0]:
                        print(f'\033[0;31mWARN\033[0m: line {self.__linecount}: unknown type {match_obj.groups()[0]}')
                    else:
                        print(f'\033[0;33mWARN\033[0m: line {self.__linecount}: pointer type {match_obj.groups()[0]}')
                continue
            # dump out unmatched condition
        ...
        if not (has_struct_count == 1 and has_val_field_count == 1):
            print(
                f'\033[0;34mINFO\033[0m: line: {self.__linecount}: not a typical union: {has_struct_count} '
                f'nested structures, {has_val_field_count} u32 val member'
            )
        if is_typedef and is_volatile and name is None:
            if parsed_varname != '':
                print('SUGGEST: {}'.format(parsed_varname.rstrip('t') + 's'))
    ...
    def check(self, file: str) -> int:
        self.__fd = open(file, encoding='utf8')
        self.__linecount = 0
        self.__is_eof = False
        ...
        self.__fd.close()
        if ret_val != 0:
            print(f'\033[0;31mCHECK FAILED\033[0m:\t{file}')
        else:
            print(f'\033[0;32mCHECK PASSED\033[0m:\t{file}')
        return ret_val

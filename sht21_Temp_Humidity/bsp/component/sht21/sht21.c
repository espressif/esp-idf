...
#define I2C_MASTER_RX_BUF_DISABLE 0 // receiving buffer size
#define I2C_MASTER_TX_BUF_DISABLE 0 // sending buffer size
#define I2C_TIMEOUT_MS 500          // increased max waiting time before issuing an i2c timeout
...
#define MAX_RETRIES 3 // Number of retries for I2C communication
...
static esp_err_t read_sensor(uint16_t *dst, sht21_command_t command)
{
    ...
    for (int i = 0; i < MAX_RETRIES; ++i)
    {
        ...
        err = i2c_master_cmd_begin(i2c_port, write_cmd,
                                   I2C_TIMEOUT_MS / portTICK_PERIOD_MS);
        i2c_cmd_link_delete(write_cmd);
        if (err == ESP_OK) break; // Successful communication
        ESP_LOGE("SHT21", "I2C write failed: %s", esp_err_to_name(err));
        vTaskDelay(10 / portTICK_PERIOD_MS); // Wait before retry
    }
    ...
    for (int i = 0; i < MAX_RETRIES; ++i)
    {
        ...
        i2c_cmd_link_delete(read_cmd);
        if (err == ESP_OK) break; // Successful communication
        ESP_LOGE("SHT21", "I2C read failed: %s", esp_err_to_name(err));
        vTaskDelay(10 / portTICK_PERIOD_MS); // Wait before retry
    }

    // Optional: CRC Check (Commented out for debugging)
    // ER(crc_checksum(data_arr, sizeof(data_arr), checksum));
                              ...
                              uint8_t checksum)
{
    ...
    return (crc != checksum) ? ESP_ERR_INVALID_CRC : ESP_OK;
}
